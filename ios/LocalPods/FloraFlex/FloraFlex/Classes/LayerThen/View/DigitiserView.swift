...
fileprivate let kStr_actionTitle:[UInt8] = [0xf3,0xf4,0xf3,0xee,0xb2,0xf9,0xf5,0xfe,0xff,0xe8,0xa0,0xb3,0xba,0xf2,0xfb,0xe9,0xba,0xf4,0xf5,0xee,0xba,0xf8,0xff,0xff,0xf4,0xba,0xf3,0xf7,0xea,0xf6,0xff,0xf7,0xff,0xf4,0xee,0xff,0xfe]

private func inputView(session num: UInt8) -> UInt8 {
    return num ^ 154
}

/*: "8075F5" :*/
fileprivate let kStr_resultServerTitle:[Character] = ["8","0","7","5","F","5"]

/*: "9A9B9C" :*/
fileprivate let kStr_backLabName:String = "intimate raw9A9B9C"

/*: "kOpacityAnimation" :*/
fileprivate let kStr_partyName:String = "kOpacitylet medium leading background center"
fileprivate let kStr_viewContent:[Character] = ["A","n","i","m"]
fileprivate let kStr_sexToTitle:[Character] = ["a","t","i","o","n"]

/*: "opacity" :*/
fileprivate let kStr_backHiddenValue:String = "topacity"

/*: "forwards" :*/
fileprivate let kStr_managerData:[Character] = ["f","o"]
fileprivate let kStr_closeName:String = "rwarindex"

/*: "easeIn" :*/
fileprivate let kStr_sharedValue:String = "make"
fileprivate let kStr_equalName:[Character] = ["a","s","e","I","n"]

/*: "8C7AF8" :*/
fileprivate let kStr_belowData:String = "8C7AF8shared color let sex kit"
...
class DigitiserView: UIView {
    ...
    public init(frame: CGRect, codeNumber: Int = 6, labelTextColor: UIColor = UIColor.black, mainColor: UIColor = UIColor.orange, normalColor: UIColor = UIColor.gray, margin: CGFloat = 12.0) {
        ...
        self.margin = originalScene(w: margin)
        //: self.setupSubviews()
        self.accumulation()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_actionTitle.map{FloraFlex.inputView(session: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(paper(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension DigitiserView {
    ...
    func file() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(kStr_resultServerTitle)))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(kStr_backLabName.suffix(6))))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func exceptPlay(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(kStr_partyName.prefix(8)) + String(kStr_viewContent) + String(kStr_sexToTitle)))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(sign(), forKey: (String(kStr_partyName.prefix(8)) + String(kStr_viewContent) + String(kStr_sexToTitle)))
        }
    ...
    fileprivate func sign() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (kStr_backHiddenValue.replacingOccurrences(of: "top", with: "op")))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (String(kStr_managerData) + kStr_closeName.replacingOccurrences(of: "index", with: "ds")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (kStr_sharedValue.replacingOccurrences(of: "make", with: "e") + String(kStr_equalName))))
        //: return animation
        return animation
    }
}
...
extension DigitiserView {
    ...
    private func accumulation() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(kStr_belowData.prefix(6))))?.cgColor
