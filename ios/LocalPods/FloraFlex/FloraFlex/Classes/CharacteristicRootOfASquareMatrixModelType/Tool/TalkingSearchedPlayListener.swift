...
/*: "%@%@" :*/
fileprivate let kStr_sectionData:String = "%@%@"

/*: "/TXCache" :*/
fileprivate let kStr_userData:String = "/TXCachesize top self"
...
// __CLOSE_PRINT__
//
//  TalkingSearchedPlayListener.swift
//  AbroadTalking
//
...
protocol LoadPlayerDelegate: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func statusMessage(player: TalkingSearchedPlayListener, status: ButtonPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func publicTransport(player: TalkingSearchedPlayListener, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func showWrite(player: TalkingSearchedPlayListener, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class TalkingSearchedPlayListener: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: LoadPlayerDelegate?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: ButtonPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = TalkingSearchedPlayListener()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(kStr_userData.prefix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(abradant), userInfo: nil, repeats: true)
...
extension TalkingSearchedPlayListener {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func cardView(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: kLet_voiceData.location6decrypt(), with: kLet_priceName)
    ...
    @objc func abradant() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.showWrite(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func map(renderMode: PathEffectRenderMode) {
    ...
    func eraseEnable(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isTalking == true {
        if ToSocketManager.shared.isTalking == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func listView() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.statusMessage(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func inheritance() -> Bool {
...
extension TalkingSearchedPlayListener: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.publicTransport(player: self, duration: Int(duration), currentTime: Int(currentTime))
