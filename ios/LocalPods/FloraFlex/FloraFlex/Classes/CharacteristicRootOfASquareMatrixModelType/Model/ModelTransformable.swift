...
/*: "id" :*/
fileprivate let kStr_topValue:[Character] = ["i","d"]
...
struct ModelTransformable: HandyJSON {
    ...
    var pic = [TheoreticalAccountTransformable]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: MomentVideoModel?
...
struct TheoreticalAccountTransformable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct MomentVideoModel: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: LowlinessContentProtocol?
}

///
//: enum MomentVideoStatus: String {
enum LowlinessContentProtocol: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension ModelTransformable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func asAdd(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(AssemblageReactiveCompatible.videoApp(text: content!, size: CGSize(width: kLet_halfData - 54, height: CGFloat(MAXFLOAT)), font: UIFont.groupSize(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == TitleMirrorPath.Photo.rawValue || momentType == TitleMirrorPath.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
