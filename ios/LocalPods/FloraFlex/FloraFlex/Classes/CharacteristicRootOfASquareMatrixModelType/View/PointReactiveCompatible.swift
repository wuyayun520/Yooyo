...
fileprivate let kStr_contentTitle:[UInt8] = [0x39,0x3e,0x39,0x44,0xf8,0x33,0x3f,0x34,0x35,0x42,0xa,0xf9,0xf0,0x38,0x31,0x43,0xf0,0x3e,0x3f,0x44,0xf0,0x32,0x35,0x35,0x3e,0xf0,0x39,0x3d,0x40,0x3c,0x35,0x3d,0x35,0x3e,0x44,0x35,0x34]

fileprivate func regularFrom(enable num: UInt8) -> UInt8 {
...
class PointReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_contentTitle.map{regularFrom(enable: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: BackgroundProgressView = {
        ...
        let v = BackgroundProgressView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(taskView(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(thrustPokeTap(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension PointReactiveCompatible {
    ...
    @objc func taskView(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func thrustPokeTap(gesture: UIPanGestureRecognizer) {
