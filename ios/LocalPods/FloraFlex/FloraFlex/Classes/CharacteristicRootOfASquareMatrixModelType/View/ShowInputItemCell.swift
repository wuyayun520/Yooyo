...
fileprivate let kStr_textSceneValue:[UInt8] = [0x83,0x88,0x83,0x8e,0x42,0x7d,0x89,0x7e,0x7f,0x8c,0x54,0x43,0x3a,0x82,0x7b,0x8d,0x3a,0x88,0x89,0x8e,0x3a,0x7c,0x7f,0x7f,0x88,0x3a,0x83,0x87,0x8a,0x86,0x7f,0x87,0x7f,0x88,0x8e,0x7f,0x7e]

fileprivate func modelPage(white num: UInt8) -> UInt8 {
...
/*: "#4FAAFF" :*/
fileprivate let kStr_equalData:String = "#4FAAFFobserver mini location with add"

/*: "reply" :*/
fileprivate let kStr_kitNextText:[Character] = ["r","e","p","l","y"]

/*: "： :*/
fileprivate let kStr_seatValue:String = "："

/*: "F5F5F5" :*/
fileprivate let kStr_commentPostValue:String = "Ficonicon5"
...
class ShowInputItemCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_textSceneValue.map{modelPage(white: $0)}, encoding: .utf8)!)
    }
...
extension ShowInputItemCell {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func senderBy(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension ShowInputItemCell {
    //: func configCell(model: TalkingCommentReplyModel) {
    func viewCellEqual(model: CharacteristicRootOfASquareMatrixModelType) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(kStr_equalData.prefix(7))))
            ...
            //: var fontA = [UIFont]()
            var fontA = [UIFont]()
            //: let font = UIFont.messageDown(fontSize: 14)
            let font = UIFont.messageDown(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (String(kStr_kitNextText)).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.statusMin(), UIColor.appTitleColor(), UIColor.statusMin(), UIColor.appTitleColor()]
                colorA = [UIColor.statusMin(), UIColor.dataFormat(), UIColor.statusMin(), UIColor.dataFormat()]
                ...
                strA = [model.nickname, "：\(model.content)"]
                //: colorA = [UIColor.statusMin(), UIColor.appTitleColor()]
                colorA = [UIColor.statusMin(), UIColor.dataFormat()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func snapHeight(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: kLet_halfData - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (kStr_commentPostValue.replacingOccurrences(of: "icon", with: "5F")))?.cgColor
...
extension ShowInputItemCell {
    ...
    private func listNextCurrent() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(senderBy(sender:)))
