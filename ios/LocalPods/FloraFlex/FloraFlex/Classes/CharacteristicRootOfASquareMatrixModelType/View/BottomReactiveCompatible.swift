...
fileprivate let kStr_targetBlockName:[UInt8] = [0x33,0x34,0x33,0x2e,0x72,0x39,0x35,0x3e,0x3f,0x28,0x60,0x73,0x7a,0x32,0x3b,0x29,0x7a,0x34,0x35,0x2e,0x7a,0x38,0x3f,0x3f,0x34,0x7a,0x33,0x37,0x2a,0x36,0x3f,0x37,0x3f,0x34,0x2e,0x3f,0x3e]

/*: "F5F5F5" :*/
fileprivate let kStr_infoText:String = "Fmodelmodel5"
...
class BottomReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_targetBlockName.map{$0^90}, encoding: .utf8)!)
    }
    ...
    func userInputCell(isListTableCell: Bool) {
    ...
    lazy var userInfoView: ImageManagerInfoView = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = ImageManagerInfoView()
    ...
    lazy var textContentView: QualityReactiveCompatible = {
        //: let  view = TalkingMomentTextContentView.init()
        let view = QualityReactiveCompatible()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var photoView: LocationViewDelegate = {
        ...
        let view = LocationViewDelegate(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: TelevisionView = {
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (kStr_infoText.replacingOccurrences(of: "model", with: "5F")))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension BottomReactiveCompatible {
    ...
    func embark(model: ModelTransformable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.transformation(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.build(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == TitleMirrorPath.Photo.rawValue || model.momentType == TitleMirrorPath.Live.rawValue {
...
//            }
            ...
            photoView.requestLive(model: cunrrenModel)
...
//            }
            ...
            videoView.configTop(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.script(model: cunrrenModel)
...
//        }
    ...
    @objc func tradeText() {
        ...
        let vc = QueryQualityViewDelegate(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != LocationThen.share.loginUserMode.sex {
        if cunrrenModel.sex != LocationThen.share.loginUserMode.sex {
    ...
    func gildTheLily() {
        ...
        if cunrrenModel.momentType == TitleMirrorPath.Video.rawValue {
    ...
    func changeResponse() {
        ...
        if videoView.isPlaying {
            //: videoView.stopPlay()
            videoView.toAGreaterExtentPlay()
        }
    }
...
extension BottomReactiveCompatible {
    ...
    private func seededPlayer() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(tradeText))
