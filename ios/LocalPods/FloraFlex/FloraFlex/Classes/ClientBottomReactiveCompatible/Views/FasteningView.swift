...
fileprivate let kStr_valueSucceedElseName:[UInt8] = [0xda,0xdf,0xda,0xe5,0x99,0xd4,0xe0,0xd5,0xd6,0xe3,0xab,0x9a,0x91,0xd9,0xd2,0xe4,0x91,0xdf,0xe0,0xe5,0x91,0xd3,0xd6,0xd6,0xdf,0x91,0xda,0xde,0xe1,0xdd,0xd6,0xde,0xd6,0xdf,0xe5,0xd6,0xd5]

fileprivate func labLeadingScale(effect num: UInt8) -> UInt8 {
...
/*: "icon_rank_coin" :*/
fileprivate let kStr_imageValue:[UInt8] = [0x75,0x7f,0x73,0x72,0x43,0x6e,0x7d,0x72,0x77,0x43,0x7f,0x73,0x75,0x72]

private func contentAt(row num: UInt8) -> UInt8 {
    return num ^ 28
}

/*: "icon_lounge" :*/
fileprivate let kStr_countValue:String = "pop lefticon_l"
fileprivate let kStr_styleVideoValue:[Character] = ["e"]

/*: "icon_rank_coin2" :*/
fileprivate let kStr_sufficientValue:String = "translatecon"
fileprivate let kStr_sizeValue:[Character] = ["o","i","n","2"]
...
class FasteningView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_valueSucceedElseName.map{labLeadingScale(effect: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        //: $0.textColor = UIColor.statusMin()
        $0.textColor = UIColor.statusMin()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .groupSize(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = OverdoTalkingButton().then {
        //: $0.setImage(UIImage.bundleBy(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.bundleBy(name: String(bytes: kStr_imageValue.map{contentAt(row: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .groupSize(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.upTo(colors: UIColor.thirdColor(), size: CGSize(width: originalScene(w: 80), height: originalScene(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.bundleBy(name: (String(kStr_countValue.suffix(6)) + "oung" + String(kStr_styleVideoValue)))
    }
}
...
extension FasteningView {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func plunge(model: PointButtonMeasurable, index: Int) {
        ...
        headPic.timeFinish(urlStr: model.headPic)
    ...
    func iconOf(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: kStr_imageValue.map{contentAt(row: $0)}, encoding: .utf8)! : (kStr_sufficientValue.replacingOccurrences(of: "translate", with: "i") + "_rank_c" + String(kStr_sizeValue))
        //: coinIconBtn.setImage(UIImage.bundleBy(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.bundleBy(name: imageStr), for: .normal)
    }
}
...
extension FasteningView {
    ...
    private func anyContentSubviews() {
    ...
    private func giftHorse() {
        ...
        coinIconBtn.snp.makeConstraints { make in
            ...
            make.trailing.equalTo(originalScene(w: -24))
            //: make.centerY.equalTo(rankLabel.snp.centerY)
            make.centerY.equalTo(rankLabel.snp.centerY)
            //: make.size.equalTo(CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33)))
            make.size.equalTo(CGSize(width: originalScene(w: 80), height: originalScene(w: 33)))
        }
    }
}
