...
fileprivate let kStr_infoIconTitle:[UInt8] = [0xfb,0x0,0xfb,0x6,0xba,0xf5,0x1,0xf6,0xf7,0x4,0xcc,0xbb,0xb2,0xfa,0xf3,0x5,0xb2,0x0,0x1,0x6,0xb2,0xf4,0xf7,0xf7,0x0,0xb2,0xfb,0xff,0x2,0xfe,0xf7,0xff,0xf7,0x0,0x6,0xf7,0xf6]

fileprivate func dataContext(time num: UInt8) -> UInt8 {
...
/*: "Not yet~" :*/
fileprivate let kStr_immediateName:[Character] = ["N","o","t"," ","y"]
fileprivate let kStr_managerName:String = "et~commit on log"

/*: "#8C7AFF" :*/
fileprivate let kStr_atName:String = "image push leading target if#8C7AFF"
...
class InheritanceDataSource: UIView {
    //: var type = 0
    var type = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_infoIconTitle.map{dataContext(time: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [PointButtonMeasurable] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(PageCollectionReactiveCompatible.self, forCellReuseIdentifier: PageCollectionReactiveCompatible.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(FasteningView.self, forCellReuseIdentifier: FasteningView.className())
    }
    ...
    lazy var tableHeaderView = ImageView(frame: CGRect(x: 0, y: 0, width: kLet_halfData, height: mediaBlock(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = SignReactiveCompatible().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(kStr_immediateName) + String(kStr_managerName.prefix(3))).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(kStr_atName.suffix(7))))
...
extension InheritanceDataSource: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: PageCollectionReactiveCompatible.className(), for: indexPath) as! PageCollectionReactiveCompatible
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: FasteningView.className(), for: indexPath) as! FasteningView
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.plunge(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.iconOf(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: PageCollectionReactiveCompatible.className()) as! PageCollectionReactiveCompatible
            //: return cell.cellHeight
            return cell.cellHeight
        }
        //: return actualHeight(h: 70)
        return mediaBlock(h: 70)
    }
...
extension InheritanceDataSource: JXSegmentedListContainerViewListDelegate {
...
extension InheritanceDataSource {
