...
fileprivate let kStr_acceptData:[UInt8] = [0x7b,0x78,0x74,0x76,0x63,0x7e,0x78,0x79,0x37,0x76,0x62,0x63,0x7f,0x78,0x65,0x7e,0x6d,0x76,0x63,0x7e,0x78,0x79,0x44,0x63,0x76,0x63,0x62,0x64,0x37,0x7e,0x64,0x37]

private func endModel(to num: UInt8) -> UInt8 {
    return num ^ 23
}

/*: "get location failed. error: :*/
fileprivate let kStr_byValue:String = "get loaction image image more"
fileprivate let kStr_sceneText:String = "on ftransition center name"
fileprivate let kStr_centerName:String = ". error:thumb self"
...
enum FromCustomReflectable: Int {
    //: case firstEntry = 0
    case firstEntry = 0
...
class PathLocationManager: NSObject {
    ...
    var locationType: FromCustomReflectable?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: PathLocationManager?
    //: class func shared() -> TalkingLocationManager {
    class func buttonRange() -> PathLocationManager {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = PathLocationManager()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func way(block: @escaping Locationblock) {
    ...
    func pairHeadBlock(block: @escaping Authblock) {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) { [weak self] in
            ...
            self.changedInfoAuthorization()
        }
    }

    /// 设备是否开启了定位服务
    //: func hasLocationService() -> Bool {
    func objectBarTo() -> Bool {
    ...
    func anIndex() -> FromCustomReflectable {
        //: switch locationPermission() {
        switch finishNortheastPermission() {
        //: case .notDetermined:
        case .notDetermined:
            //: return .firstEntry
            return .firstEntry
    ...
    func finishNortheastPermission() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            pathLine(message: String(bytes: kStr_acceptData.map{endModel(to: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension PathLocationManager: CLLocationManagerDelegate {
    ...
    private func changedInfoAuthorization() {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        pathLine(message: (String(kStr_byValue.prefix(6)) + "cati" + String(kStr_sceneText.prefix(4)) + "ailed" + String(kStr_centerName.prefix(8))) + "\(error.localizedDescription)")
    }
}
