...
fileprivate let kStr_modelText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_match_shadow" :*/
fileprivate let kStr_bottomText:[Character] = ["i","m","g","_","m","a","t","c","h","_","s","h","a","d","o"]
fileprivate let kStr_managerViewBlockValue:String = "equal"

/*: "icon_home_girl" :*/
fileprivate let kStr_transformContent:[Character] = ["i","c","o","n","_","h","o","m","e","_","g","i"]
fileprivate let kStr_titleLabName:[Character] = ["r","l"]

/*: "icon_home_v" :*/
fileprivate let kStr_cellValue:[Character] = ["i","c","o","n","_","h","o","m"]
fileprivate let kStr_faceName:String = "class adjust info usere_v"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let kStr_viewValue:String = "medium"
fileprivate let kStr_layerText:[Character] = ["n","_","d","y","n","a","m","i","c","_","s","t","o","p","_","n","o","r"]

/*: "#2DF2FF" :*/
fileprivate let kStr_addTitleData:String = "#2DF2FFfalse record"

/*: "· :*/
fileprivate let kStr_appArrayName:[Character] = ["·"]

/*: "img_home_shadow" :*/
fileprivate let kStr_pictureValue:String = "file on content viewimg_"
fileprivate let kStr_quickMeText:String = "HADOW"

/*: "icon_home_boy" :*/
fileprivate let kStr_countTitle:String = "icon_hcolor let for content error"

/*: "FinancialAidPlayerDelegate" :*/
fileprivate let kStr_lastValue:[Character] = ["T","U","s","e","r"]
fileprivate let kStr_normalData:String = "Detmake var self"
fileprivate let kStr_countValue:String = "ailHcell in size equal"
fileprivate let kStr_indexGestureName:String = "label namerIte"

/*: "#FFFFFF" :*/
fileprivate let kStr_modelName:String = "#FFFFFFload false theory"
...
class CourseThen: UIView {
    ...
    private var model = BottomResultModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_modelText.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var gradientShadowImageView: UIImageView = {
        ...
        //: imageView.image = UIImage.bundleBy(name: "img_match_shadow")
        imageView.image = UIImage.bundleBy(name: (String(kStr_bottomText) + kStr_managerViewBlockValue.replacingOccurrences(of: "equal", with: "w")))
        //: return imageView
        return imageView
        //: }()
    }()
    ...
    private lazy var sexImgV: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.bundleBy(name: "icon_home_girl")
        img.image = UIImage.bundleBy(name: (String(kStr_transformContent) + String(kStr_titleLabName)))
    ...
    private lazy var authImgV: UIImageView = {
        ...
        img.image = UIImage.bundleBy(name: (String(kStr_cellValue) + String(kStr_faceName.suffix(3))))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: return img
        return img
    ...
    lazy var playImageView: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.bundleBy(name: (kStr_viewValue.replacingOccurrences(of: "medium", with: "bt") + String(kStr_layerText))))
        //: imag.isHidden = true
        imag.isHidden = true
        //: return imag
        return imag
...
//        v.roundBgColor = UIColor.white
        ...
        v.progressColor = UIColor(hex: (String(kStr_addTitleData.prefix(7))))!
        //: return v
        return v
        //: }()
    }()
...
extension CourseThen {
    /// 更新视图UI
    /// - Parameter model: 数据模型
    //: func updatePicturesView(model: TalkingMatchResultModel) {
    func errorAdd(model: BottomResultModel) {
        //: self.model = model
        self.model = model
        ...
        ageLab.text = "·\(model.age)"
        //: userImg.setUrlImage(urlStr: model.headPic, placeImg: UIImage.bundleBy(name: "img_home_shadow"))
        userImg.failureFinish(urlStr: model.headPic, placeImg: UIImage.bundleBy(name: (String(kStr_pictureValue.suffix(4)) + "home_s" + kStr_quickMeText.lowercased())))
        //: if model.sex == Gender.male.rawValue {
        if model.sex == SucceedComparable.male.rawValue {
            //: sexImgV.image = UIImage.bundleBy(name: "icon_home_boy")
            sexImgV.image = UIImage.bundleBy(name: (String(kStr_countTitle.prefix(6)) + "ome_boy"))
            //: } else {
        } else {
            //: sexImgV.image = UIImage.bundleBy(name: "icon_home_girl")
            sexImgV.image = UIImage.bundleBy(name: (String(kStr_transformContent) + String(kStr_titleLabName)))
        }
        ...
        self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
            ...
            self.progressView.miniTime(totalTime: self.MatchResultProgress_Timer, currTime: time)
            //: time -= 1
            time -= 1
            //: }, repeats: true)
        }, repeats: true)
...
extension CourseThen: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: FinancialAidPlayerDelegate.self, reuseIdentifier: (String(kStr_lastValue) + String(kStr_normalData.prefix(3)) + String(kStr_countValue.prefix(4)) + "eade" + String(kStr_indexGestureName.suffix(4)) + "mCell"))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && self.model.videoUrl.count > 1 { // 视频
            ...
            bannerCell.income(videoUrl: self.model.videoUrl, imgUrl: self.model.cover)
            /// 自动播放完视频后滑动到下一个
...
//                }
            ...
            bannerCell.bgImgView.failureFinish(urlStr: str, placeImg: UIImage.infoPlayer())
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(kStr_modelName.prefix(7))))!.withAlphaComponent(1.0)
        //: pageControl.inactiveColor = UIColor(hex: "#FFFFFF")!.withAlphaComponent(0.6)
        pageControl.inactiveColor = UIColor(hex: (String(kStr_modelName.prefix(7))))!.withAlphaComponent(0.6)
...
extension CourseThen: JXBannerDelegate {
