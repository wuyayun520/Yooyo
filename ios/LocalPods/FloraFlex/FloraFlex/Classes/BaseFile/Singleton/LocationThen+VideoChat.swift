...
/*: "type" :*/
fileprivate let kStr_moreTitle:String = "commentype"

/*: "You are currently on a call, please try again later" :*/
fileprivate let kStr_spaceName:[UInt8] = [0x72,0x65,0x74,0x61,0x6c,0x20,0x6e,0x69,0x61,0x67,0x61,0x20,0x79,0x72,0x74,0x20,0x65,0x73,0x61,0x65,0x6c,0x70,0x20,0x2c,0x6c,0x6c,0x61,0x63,0x20,0x61,0x20,0x6e,0x6f,0x20,0x79,0x6c,0x74,0x6e,0x65,0x72,0x72,0x75,0x63,0x20,0x65,0x72,0x61,0x20,0x75,0x6f,0x59]

/*: "onCallResume" :*/
fileprivate let kStr_infoName:String = "label"
fileprivate let kStr_topFromName:[Character] = ["n","C","a","l","l","R","e","s","u","m","e"]
...
extension LocationThen {
    ...
    func startCover(info: [String: Any], situation: LineChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(kStr_moreTitle.replacingOccurrences(of: "comment", with: "t"))].intValue
...
extension LocationThen {
    ...
    private func scoop(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isTalking {
        if !resumeTalk && ToSocketManager.shared.isTalking {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.detailDownArray(showMsg: String(bytes: kStr_spaceName.reversed(), encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if giftFew()!.isKind(of: ShadowErrorDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            giftFew()!.isKind(of: SucceedRecognizerDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            giftFew()!.isKind(of: ColourationViewDelegate.self)
        {
            //: return
            return
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! ShadowErrorDelegate).chatModel = TitleGiftChatModel.colorLoad(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = SucceedRecognizerDelegate()
            ...
            (vc as! SucceedRecognizerDelegate).chatModel = TitleGiftChatModel.colorLoad(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(kStr_infoName.replacingOccurrences(of: "label", with: "o") + String(kStr_topFromName))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.counternalRepresentation(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func counternalRepresentation(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = giftFew() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = TableMacroDefine.getWindow().rootViewController as! UITabBarController
            let rootVc = TableMacroDefine.oeilDeBoeufView().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(SucceedRecognizerDelegate.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(ShadowErrorDelegate.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: ShadowErrorDelegate?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! ShadowErrorDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? ShadowErrorDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.modifyAndData(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: EngineControllerDelegate.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! EngineControllerDelegate
            ...
            if topVc != nil, topVc!.isKind(of: BackgroundReactiveCompatible.self) {
            ...
            var currentChatVc: SucceedRecognizerDelegate?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! SucceedRecognizerDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? SucceedRecognizerDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.clickData(resumeData: resumeData)
                }
                //: return
                return
            }
