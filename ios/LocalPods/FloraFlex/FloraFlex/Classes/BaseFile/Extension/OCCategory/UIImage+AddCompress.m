...
@implementation UIImage (AddCompress)
...
- (UIImage *)gift {
    ...
    return [UIImage imageWithData:[self goaBean]];
}
...
- (UIImage *)price:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self keyCurrent:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self render:size];
...
- (CGSize)keyCurrent:(Boolean)isSession {
...
+ (NSData *)pickDoing:(UIImage *)image local:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image telecasting:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG smart:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG patternDescription:scalesize burnVideo:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG smart:(fileSize*1024)];
    }
...
- (UIImage*)telecasting:(CGSize)targetSize{
...
- (UIImage *)patternDescription:(CGSize)targetSize burnVideo:(BOOL)isScale{
...
- (NSData *)smart:(NSInteger)fileSize{
