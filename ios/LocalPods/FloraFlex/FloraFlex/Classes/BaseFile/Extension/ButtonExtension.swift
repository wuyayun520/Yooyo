...
/*: "recharge" :*/
fileprivate let kStr_showData:String = "RECHARG"
fileprivate let kStr_iconValue:String = "lab"

/*: "recharge_ :*/
fileprivate let kStr_cameraTitle:[Character] = ["r","e","c","h"]
fileprivate let kStr_whiteColorThenValue:String = "arge_lab player title for none"

/*: ".png" :*/
fileprivate let kStr_afterValue:String = "save let self.png"

/*: "_ :*/
fileprivate let kStr_styleData:[Character] = ["_"]

/*: .png" :*/
fileprivate let kStr_informAsideViewData:String = "in above.png"

/*: ".jpg" :*/
fileprivate let kStr_shadowFemaleName:String = "background var model user head.jpg"

/*: .jpg" :*/
fileprivate let kStr_halfMakeTitle:String = ".jpgbackground in view size label"
...
enum ViewManagerNumber: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class OverdoTalkingButton: UIButton {
    ...
    var imageAlignment: ViewManagerNumber = .left
...
extension UIButton {
    ...
    func userTitle(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        alignFrom(interval: interval, topOffset: 0)
    }
    ...
    func alignFrom(interval: CGFloat, topOffset: CGFloat) {
    ...
    func valid(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func colorEffectLoad(color: UIColor, forState: UIControl.State) {
    ...
    func displayFinish(urlStr: String,
    ...
    func dataFile(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((kStr_showData.lowercased() + kStr_iconValue.replacingOccurrences(of: "lab", with: "e"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (kStr_showData.lowercased() + kStr_iconValue.replacingOccurrences(of: "lab", with: "e")), with: (String(kStr_cameraTitle) + String(kStr_whiteColorThenValue.prefix(5))) + "\(kLet_guideDoingTitle)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.displayFinish(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(kStr_afterValue.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(kStr_afterValue.suffix(4))), with: "_\(kLet_guideDoingTitle)" + (String(kStr_informAsideViewData.suffix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(kStr_shadowFemaleName.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(kStr_shadowFemaleName.suffix(4))), with: "_\(kLet_guideDoingTitle)" + (String(kStr_halfMakeTitle.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.displayFinish(urlStr: newUrl, placeImg: nil)
    }
