...
class SubReactiveCompatible: UIViewController {
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = TableMacroDefine.getScreenHeight() - (topDistance + kTitle_H)
        var height = TableMacroDefine.appearSumeraction() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= kLet_screenContent
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: TableMacroDefine.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: TableMacroDefine.equalWidth(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(TableMacroDefine.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(TableMacroDefine.equalWidth()), height: 0)
        ...
        scrollView.semanticContentAttribute = DeleteClientThen.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: TableMacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: TableMacroDefine.equalWidth()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: TableMacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: TableMacroDefine.equalWidth()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = DeleteClientThen.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(sizeCounto), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = TableMacroDefine.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = TableMacroDefine.equalWidth() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(TableMacroDefine.getScreenWidth()-originX-titleWidth)
            let x = DeleteClientThen.shared.direction == .leftToRight ? originX:(TableMacroDefine.equalWidth()-originX-titleWidth)
...
extension SubReactiveCompatible {
    ...
    @objc private func sizeCounto(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        upwardlySend(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * TableMacroDefine.getScreenWidth()
        let offset_x = CGFloat(index) * TableMacroDefine.equalWidth()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        markIndex(withIndex: index)
        //: seletedViewController(withIndex: index)
        seleted(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func markIndex(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * TableMacroDefine.getScreenWidth()
        let origin_x = CGFloat(index) * TableMacroDefine.equalWidth()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: TableMacroDefine.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: TableMacroDefine.equalWidth(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func upwardlySend(_ button: UIButton) {
    ...
    func nameEqual(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= TableMacroDefine.getScreenWidth() {
        if titleScrollView.contentSize.width <= TableMacroDefine.equalWidth() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - TableMacroDefine.getScreenWidth() * 0.5
        var offset_x = sender.center.x - TableMacroDefine.equalWidth() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - TableMacroDefine.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - TableMacroDefine.equalWidth()
...
extension SubReactiveCompatible {
    ...
    func seleted(withIndex index: Int) {
        ...
        delegate?.invisible(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension SubReactiveCompatible: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / TableMacroDefine.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / TableMacroDefine.equalWidth())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            upwardlySend(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        markIndex(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / TableMacroDefine.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / TableMacroDefine.equalWidth())
        ...
        //: let scaleR = scrollView.contentOffset.x / TableMacroDefine.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / TableMacroDefine.equalWidth() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
