...
fileprivate let kStr_equalName:[UInt8] = [0xc5,0xca,0xc5,0xd0,0x84,0xbf,0xcb,0xc0,0xc1,0xce,0x96,0x85,0x7c,0xc4,0xbd,0xcf,0x7c,0xca,0xcb,0xd0,0x7c,0xbe,0xc1,0xc1,0xca,0x7c,0xc5,0xc9,0xcc,0xc8,0xc1,0xc9,0xc1,0xca,0xd0,0xc1,0xc0]

fileprivate func mInvite(end num: UInt8) -> UInt8 {
...
typealias SlidingHandler = (_ direction: LineSlideDirection) -> Void
...
class ValueReactiveCompatible: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (kLet_successText - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_equalName.map{mInvite(end: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.viewNumber(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: ClientOverlayView = {
        ...
        let overlay = ClientOverlayView(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension ValueReactiveCompatible {
    ...
    func actionPostion() {
        ...
        self.overlay.bindBy(startX: kLet_successText - x)
    }
    ...
    private func viewNumber(point: CGFloat) -> UIImage {
...
extension ValueReactiveCompatible: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = kLet_successText - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.proveComponent(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.proveComponent(isShow: true)
    }
...
extension ValueReactiveCompatible {
    ...
    private func centerSub() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(kLet_successText)
