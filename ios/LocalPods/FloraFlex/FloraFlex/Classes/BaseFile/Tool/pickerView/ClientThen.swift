...
/*: "Select a date" :*/
fileprivate let kStr_name:String = "equal"
fileprivate let kStr_appText:String = "ELECT"

/*: M" :*/
fileprivate let kStr_requestValue:[Character] = ["M"]

/*: D" :*/
fileprivate let kStr_successText:String = "format"

/*: Y" :*/
fileprivate let kStr_toMatchData:[Character] = ["Y"]
...
protocol UpVoiceObjectProtocol: NSObjectProtocol {
...
class ClientThen: PageDataThen, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: UpVoiceObjectProtocol?
    ...
    override func pathGesture() {
        ...
        titleString = (kStr_name.replacingOccurrences(of: "equal", with: "S") + kStr_appText.lowercased() + " a date").localized
        ...
        tablePlaceUpright(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func tablePlaceUpright(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func nearSnap(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.array(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.array(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return digitiserCell(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func digitiserCell(year: Int, month: Int) -> Int {
