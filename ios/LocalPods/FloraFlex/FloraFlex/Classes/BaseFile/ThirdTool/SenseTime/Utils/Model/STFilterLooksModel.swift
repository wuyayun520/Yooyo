...
extension STFilterLooksModel {
    ...
    func getSourceFromPath(bundleName: String, bmpType: STEffectsType) -> [STFilterLooksModel] {
        ...
        let strLocalBundlePath = GearingPodspecBundle.bundle.path(forResource: bundleName, ofType: "bundle")
        ...
        for i in 0...(arrFiles?.count ?? 1)-1 {
            ...
            if strFileName.hasSuffix(".zip") {
                ...
                if imageThumb == nil {
                    imageThumb = UIImage.bundleBy(name: "st_none")
                }
    ...
    func getFilterBundleSource(bundleName: NSString) -> [STFilterLooksModel] {
        let arrFiles = try? FileManager.default.contentsOfDirectory(atPath: TableMacroDefine.postulation())
        ...
        for i in 0...(arrFiles?.count ?? 1)-1 {
            ...
            if strFileName.hasSuffix("model") {
                ...
                if orignName.count > 0 {
                    sourceDict[orignName] = NSString.path(withComponents: [TableMacroDefine.postulation(), strFileName ])
                }
            }
        }
