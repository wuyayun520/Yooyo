...
/*: "http://stat. :*/
fileprivate let kStr_fileValue:String = "title gift adhttp:"
fileprivate let kStr_reasonValue:String = "//stat.model make"

/*: .com" :*/
fileprivate let kStr_playerName:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let kStr_followingText:String = "text kindhttp://"

/*: "https://" :*/
fileprivate let kStr_labelName:[Character] = ["h","t","t","p","s"]
fileprivate let kStr_equalValue:[Character] = [":","/","/"]

/*: "http://m. :*/
fileprivate let kStr_cellValue:[Character] = ["h","t","t","p",":","/","/"]
fileprivate let kStr_colorBottomText:[Character] = ["m","."]

/*: "http:// :*/
fileprivate let kStr_innerRawTrueText:[Character] = ["h","t","t","p",":","/","/"]

/*: "-1" :*/
fileprivate let kStr_timeData:String = "-1"

/*: "3,90" :*/
fileprivate let kStr_frameGroupValue:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let kStr_infoName:String = "enter"
fileprivate let kStr_totalTitle:String = "990-0"

/*: "MM-dd-yyyy" :*/
fileprivate let kStr_moreGiftData:[Character] = ["M","M","-","d","d","-","y","y","y","y"]

/*: "id" :*/
fileprivate let kStr_scaleData:String = "play"
...
class SizeRequestModel: NSObject {
    ...
    var requestType: ButtonRequestType?
    ...
    override init() {
        ...
        self.requestServer = ViewManagerAddrTool.share.serverUrlStr
...
struct MiniBaseResponse: HandyJSON {
    //: var errno: Int!
...
public struct CanErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct CollectionUpConfigModel: HandyJSON {
    ...
    var CSConfig = FibrousnessHandyJSON() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(kStr_fileValue.suffix(5)) + String(kStr_reasonValue.prefix(7))) + "\(kLet_priceName)" + (String(kStr_playerName))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(kStr_followingText.suffix(7)))) && !statDomain.hasPrefix((String(kStr_labelName) + String(kStr_equalValue))) {
                //: return "http://".appending(statDomain)
                return (String(kStr_followingText.suffix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(kStr_cellValue) + String(kStr_colorBottomText)) + "\(kLet_priceName)" + (String(kStr_playerName))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(kStr_innerRawTrueText)) + "\(urlDomain)"
        }
    }
...
public class OmitModelType: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [ToMeasurable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [ToMeasurable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [ToMeasurable] = [] // 语音收费设置
    ...
    var vipSetting = [AssemblageHandyJSON]() // vip皮肤
    ...
    var tabPageBanner = [RawDataBannerModel]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = IsoclinicTransformable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class EqualMeasurable: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [NameTagTransformable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [NameTagTransformable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var callPopupSetting = (kStr_frameGroupValue.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.systemImage(self.birthday ?? (kStr_infoName.replacingOccurrences(of: "enter", with: "1") + kStr_totalTitle.capitalized + "1-01"), dateFormat: (String(kStr_moreGiftData)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.dropHalf().year else { return "0" }
...
struct PathTransformable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct NameTagTransformable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func mistake(compareTo: NameTagTransformable) -> Bool {
        //: return
        return
...
struct LoadInfoModel {
    ...
    mutating func somebodySelected() {
...
struct UpwardModelType: HandyJSON {
    ...
    var aboutMe: [LineTypeModel]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [LineTypeModel]!
}

//: struct UserTagTypeModel: HandyJSON {
struct LineTypeModel: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [NameTagTransformable]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct FibrousnessHandyJSON: HandyJSON {
...
struct AssemblageHandyJSON: HandyJSON {
...
struct IsoclinicTransformable: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
