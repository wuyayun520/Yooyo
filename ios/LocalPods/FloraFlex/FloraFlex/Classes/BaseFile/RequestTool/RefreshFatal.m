...
Byte kStr_giftPositionData[] = {97, 21, 3, 115, 105, 104, 116, 95, 115, 105, 95, 107, 99, 117, 102, 95, 101, 104, 116, 95, 116, 97, 104, 119, 161};


//: -----BEGIN PUBLIC KEY-----
Byte kStr_frameData[] = {26, 26, 10, 230, 45, 24, 91, 237, 187, 200, 45, 45, 45, 45, 45, 89, 69, 75, 32, 67, 73, 76, 66, 85, 80, 32, 78, 73, 71, 69, 66, 45, 45, 45, 45, 45, 127};


//: -----END PUBLIC KEY-----
Byte kStr_modelData[] = {32, 24, 6, 89, 110, 58, 45, 45, 45, 45, 45, 89, 69, 75, 32, 67, 73, 76, 66, 85, 80, 32, 68, 78, 69, 45, 45, 45, 45, 45, 5};
...
+ (SecKeyRef)coverContentKey:(NSString *)key{
    //: NSRange spos = [key rangeOfString:@"-----BEGIN PUBLIC KEY-----"];
    NSRange spos = [key rangeOfString:StringFromParentData(kStr_frameData)];
    //: NSRange epos = [key rangeOfString:@"-----END PUBLIC KEY-----"];
    NSRange epos = [key rangeOfString:StringFromParentData(kStr_modelData)];
    ...
    NSString *tag = StringFromParentData(kStr_giftPositionData);
...
+ (NSString *)head:(NSString *)str size:(NSString *)pubKey{
    ...
    NSData *data = [RefreshFatal data:[str dataUsingEncoding:NSUTF8StringEncoding] beam:pubKey];
...
+ (NSData *)data:(NSData *)data beam:(NSString *)pubKey{
    ...
    SecKeyRef keyRef = [RefreshFatal coverContentKey:pubKey];
    //: if(!keyRef) {
    if(!keyRef) {
        //: return nil;
        return nil;
...
+ (NSString *)decryptAndPassee:(NSString *)str imageKey:(NSString *)pubKey{
    ...
    data = [RefreshFatal methodProduce:data textKey:pubKey];
    //: NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    //: return ret;
    return ret;
...
Byte * ParentDataToCache(Byte *data) {
    int with = data[0];
    int user = data[1];
    int nomDeGuerre = data[2];
    if (!with) return data + nomDeGuerre;
    for (int i = 0; i < user / 2; i++) {
        int begin = nomDeGuerre + i;
        int end = nomDeGuerre + user - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[nomDeGuerre + user] = 0;
    return data + nomDeGuerre;
}
