...
/*: "&sr=6" :*/
fileprivate let kStr_textValue:[Character] = ["&","s","r","=","6"]
...
enum SelectedComparable: String, CaseIterable {
...
enum VariantLayerNameProtocol: String {
...
class ContentThen: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func inviteType(urlType: VariantLayerNameProtocol) -> String {
        //: var url = LocationThen.share.appConfigMode.urlH5Domain
        var url = LocationThen.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(LocationThen.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : LocationThen.share.appUserConfigMode.fullRechargePage)
            url.append(LocationThen.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : LocationThen.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(LocationThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : LocationThen.share.appUserConfigMode.halfRechargePage)
            url.append(LocationThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : LocationThen.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = LocationThen.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = LocationThen.share.appUserConfigMode.halfRechargePage.appending((String(kStr_textValue)))
            //: url.append(LocationThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(LocationThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
