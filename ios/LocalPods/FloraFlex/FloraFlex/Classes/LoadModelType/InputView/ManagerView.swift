...
fileprivate let kStr_pathText:[Character] = ["\u{8bbe}","\u{7f6e}","普","通","\u{72b6}"]
fileprivate let kStr_failureData:String = "态的文字颜色"
fileprivate let kStr_locationData:String = "\u{7528}RGB"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let kStr_formatValue:String = "\u{8bbe}置\u{9009}中状态"
fileprivate let kStr_interactionData:[Character] = ["时"," ","请","\u{4f7f}","\u{7528}","R","G","B","空","间","\u{7684}","颜","色","值"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let kStr_selectedData:[UInt8] = [0x58,0x5d,0x58,0x63,0x17,0x52,0x5e,0x53,0x54,0x61,0x29,0x18,0xf,0x57,0x50,0x62,0xf,0x5d,0x5e,0x63,0xf,0x51,0x54,0x54,0x5d,0xf,0x58,0x5c,0x5f,0x5b,0x54,0x5c,0x54,0x5d,0x63,0x54,0x53]

fileprivate func noOf(pic num: UInt8) -> UInt8 {
...
/*: "设置的下标不合法!!" :*/
fileprivate let kStr_endContent:[Character] = ["设","置","的","下","标","不"]
fileprivate let kStr_priceValue:[Character] = ["\u{5408}","\u{6cd5}","!","!"]

/*: "Bags" :*/
fileprivate let kStr_selectedText:[Character] = ["B","a","g","s"]
...
open class ManagerView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: FailureSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.modelBurn(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(kStr_pathText) + kStr_failureData.capitalized + "时 请使" + kStr_locationData + "空间的颜色值"))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.modelBurn(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((kStr_formatValue + "的文字颜色" + String(kStr_interactionData)))
        }
    ...
    fileprivate func modelBurn(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: FailureSegmentStyle, titles: [String]) {
    ...
    @objc func viewBegin(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? CustomReactiveCompatible else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_selectedData.map{noOf(pic: $0)}, encoding: .utf8)!)
    }
}
...
public extension ManagerView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func application(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(kStr_endContent) + String(kStr_priceValue)))
...
extension ManagerView {
    ...
    fileprivate func nearSmall() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = CustomReactiveCompatible(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.viewBegin(_:)))
            ...
            if title == (String(kStr_selectedText)).localized {
    ...
    fileprivate func setAtLeftStatus() {
        ...
        if let firstLabel = labelsArray[0] as? CustomReactiveCompatible {
    ...
    fileprivate func turfAndLineScrollUp() {
    ...
    public func conversion(_ currentIndex: Int) {
...
open class CustomReactiveCompatible: UILabel {
    /// 用来记录当前label的缩放比例
