...
/*: "call_bgm" :*/
fileprivate let kStr_dataTitle:String = "call_bgmview height"

/*: "call_response_bgm" :*/
fileprivate let kStr_backgroundTitle:String = "call_if let gender var type"
fileprivate let kStr_timeValue:[Character] = ["n","s","e","_","b","g","m"]
...
enum LocationPropertyProtocol: Int {
    //: case Call
    case Call /// 音视频拨打音乐
    //: case Response
...
class ChorusPlayListener: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = LocationPropertyProtocol.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func writerSName(type: LocationPropertyProtocol) -> ChorusPlayListener {
        //: let player = TalkingCallBGMPlayer()
        let player = ChorusPlayListener()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.hadith()
...
extension ChorusPlayListener {
    ...
    private func showClick(notify: Notification) {
    ...
    private func viewNotify(notify: Notification) {
    ...
    private func hadith() {
        ...
        var fileName = (String(kStr_dataTitle.prefix(8)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(kStr_backgroundTitle.prefix(5)) + "respo" + String(kStr_timeValue))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = LoadGrappleEffectTool.default.postOffType(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension ChorusPlayListener: TXVodPlayListener {
