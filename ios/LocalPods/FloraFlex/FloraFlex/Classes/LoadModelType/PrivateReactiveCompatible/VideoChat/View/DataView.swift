...
fileprivate let kStr_equalName:[UInt8] = [0xf6,0xf1,0xf6,0xeb,0xb7,0xfc,0xf0,0xfb,0xfa,0xed,0xa5,0xb6,0xbf,0xf7,0xfe,0xec,0xbf,0xf1,0xf0,0xeb,0xbf,0xfd,0xfa,0xfa,0xf1,0xbf,0xf6,0xf2,0xef,0xf3,0xfa,0xf2,0xfa,0xf1,0xeb,0xfa,0xfb]

private func returnEqual(view num: UInt8) -> UInt8 {
    return num ^ 159
}
...
@objc protocol ButtonObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func equalScroll()
...
class DataView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(colorAt))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_equalName.map{returnEqual(view: $0)}, encoding: .utf8)!)
    }
...
extension DataView {
    ...
    func aboveExecute() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(pathEvent), userInfo: nil, repeats: true)
    }
...
extension DataView {
    ...
    func mixInEqualView(msgModel: ShareTransformable?) {
    ...
    func analogDigitalConverterBottom() {
        // 处于爬楼状态更新更多按钮
        //: if !self.inPending {
        if !self.inPending {
    ...
    func drunkImage() {
        ...
        if AllHeight > kLet_messageValue {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < kLet_messageValue {
                ...
                self.msgTableView.height = kLet_messageValue
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kLet_messageValue - AllHeight
        ...
        astatine(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func astatine(animated: Bool) {
    ...
    func upwardlyPending(inPending: Bool) {
...
extension DataView: UITableViewDelegate, UITableViewDataSource {
    ...
    func ofMake(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kLet_backValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = DataWithReactiveCompatible.route(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension DataView {
    ...
    func deviseSubviews() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: kLet_appFirstValue, height: kLet_messageValue)
    }
}
