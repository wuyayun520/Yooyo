...
fileprivate let kStr_viewByData:[UInt8] = [0x14,0x13,0x14,0x9,0x55,0x1e,0x12,0x19,0x18,0xf,0x47,0x54,0x5d,0x15,0x1c,0xe,0x5d,0x13,0x12,0x9,0x5d,0x1f,0x18,0x18,0x13,0x5d,0x14,0x10,0xd,0x11,0x18,0x10,0x18,0x13,0x9,0x18,0x19]

private func trancheTarget(fill num: UInt8) -> UInt8 {
    return num ^ 125
}
...
// __CLOSE_PRINT__
//
//  ViewValueReactiveCompatible.swift
//  AbroadTalking
//
...
@objc protocol InputObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func statusUser()

    //: @objc optional func endScroll()
    @objc optional func endColorReply()
}

//: class TalkingDanmuMsgListTableView: UIView {
class ViewValueReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_viewByData.map{trancheTarget(fill: $0)}, encoding: .utf8)!)
    }
...
extension ViewValueReactiveCompatible {
    //: func startTimer() {
    func statusValue() {
        //: stopTimer()
        mixIn()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(clickEvent), userInfo: nil, repeats: true)
    }
...
extension ViewValueReactiveCompatible {
    ...
    func skipModel(msgModel: BackgroundMeasurable?) {
    ...
    func drugged() {
        ...
        if AllHeight > kLet_lengthName {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < kLet_lengthName {
                ...
                self.msgTableView.height = kLet_lengthName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kLet_lengthName - AllHeight
        ...
        upwardAnimated(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func upwardAnimated(animated: Bool) {
    ...
    func viewRemove(inPending: Bool) {
...
extension ViewValueReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func firstNameWith(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.endColorReply?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kLet_deviceValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = EraseBlockThen.marginOfError(tableView: tableView, msg: msgModel, indexPath: indexPath)
