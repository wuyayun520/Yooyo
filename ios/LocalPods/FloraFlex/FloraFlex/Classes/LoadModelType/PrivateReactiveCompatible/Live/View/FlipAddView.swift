...
fileprivate let kStr_totalDataValue:[UInt8] = [0xfa,0xfd,0xfa,0xe7,0xbb,0xf0,0xfc,0xf7,0xf6,0xe1,0xa9,0xba,0xb3,0xfb,0xf2,0xe0,0xb3,0xfd,0xfc,0xe7,0xb3,0xf1,0xf6,0xf6,0xfd,0xb3,0xfa,0xfe,0xe3,0xff,0xf6,0xfe,0xf6,0xfd,0xe7,0xf6,0xf7]

private func toLabel(index num: UInt8) -> UInt8 {
    return num ^ 147
}

/*: "no data available" :*/
fileprivate let kStr_shareData:String = "no dimage title return image name"
fileprivate let kStr_containerTitle:String = "imageailab"
fileprivate let kStr_tapData:String = "lpast"

/*: "list" :*/
fileprivate let kStr_socialValue:[Character] = ["l","i","s","t"]
...
class FlipAddView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_totalDataValue.map{toLabel(index: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(pathImage), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var emptyView: UILabel = {
        ...
        lab.text = (String(kStr_shareData.prefix(4)) + "ata " + kStr_containerTitle.replacingOccurrences(of: "image", with: "av") + kStr_tapData.replacingOccurrences(of: "past", with: "e")).localized
        //: lab.textColor = .white
        lab.textColor = .white
...
extension FlipAddView {
    ...
    private func spectralColorList() {
        ...
        LoadEqualRequestTool.jointWith(category: 2) { [weak self] _, result, _ in
            ...
            json[(String(kStr_socialValue))].arrayObject?.forEach { dict in
...
extension FlipAddView {
    ...
    private func responseHeight() -> CGFloat {
        ...
        return lineNum * kLet_name + 24.0 + kLet_formatNetValue
    }
}
...
extension FlipAddView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.viewDismiss(model: dataArr[indexPath.row])
        //: return cell
        return cell
    }
...
extension FlipAddView {
    ...
    func withOmit(from: SucceedFromType) {
        //: switch from {
        switch from {
        //: case .LiveRoom:
        case .LiveRoom:
            //: dataArr = TalkingLiveManager.shared().liveRoomModel.gameList
            dataArr = VideoPushListener.isGesture().liveRoomModel.gameList
            //: showAnimation()
            showBlock()
    ...
    private func showBlock() {
        ...
        self.contentView.frame = CGRect(x: 0, y: kLet_noSeatName, width: kLet_halfData, height: responseHeight())
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y -= self.contentView.height
            self.contentView.frame.origin.y -= self.contentView.height
...
extension FlipAddView {
    ...
    private func random() {
        ...
        contentView.frame = CGRect(x: 0, y: kLet_noSeatName, width: kLet_halfData, height: responseHeight())
