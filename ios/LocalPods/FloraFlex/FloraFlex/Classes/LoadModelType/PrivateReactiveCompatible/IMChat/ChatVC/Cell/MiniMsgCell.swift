...
fileprivate let kStr_data:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "all" :*/
fileprivate let kStr_errorCenterData:String = "alivelive"
...
class MiniMsgCell: CollectionViewCell {
    //: var giftData: GiftPathCellData?
    var giftData: GiftPathCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_data.reversed(), encoding: .utf8)!)
    }
...
extension MiniMsgCell {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: self.giftData = data as? GiftPathCellData
        self.giftData = data as? GiftPathCellData
        ...
        self.giftImgView.timeFinish(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (kStr_errorCenterData.replacingOccurrences(of: "live", with: "l")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.bundleBy(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.bundleBy(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.timeFinish(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
