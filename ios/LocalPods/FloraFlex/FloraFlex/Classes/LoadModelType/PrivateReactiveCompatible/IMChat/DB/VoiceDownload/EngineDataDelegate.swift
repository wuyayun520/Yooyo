...
fileprivate let kStr_normalData:[Character] = ["D","o","c","u","m","e","n","t","s","/","U","s","e","r","/","v","o","i","c"]
fileprivate let kStr_leadingRangeData:String = "size gesture style managere/"

/*: "Documents/User/record/" :*/
fileprivate let kStr_toData:String = "Documarray type top price"
fileprivate let kStr_pathValue:String = "readnt"
fileprivate let kStr_infoName:[Character] = ["s","/","U","s","e","r","/","r","e","c","o","r","d","/"]

/*: "cancelled" :*/
fileprivate let kStr_shData:String = "canctitlel"
fileprivate let kStr_commentData:[Character] = ["e","d"]

/*: .wav" :*/
fileprivate let kStr_itemViewText:String = "if equal let session.wav"
...
typealias OneKeyGreetDownloadBlock = (_ model: EqualTaskModel) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol SubManagerDelegate: NSObjectProtocol {
    /// 下载完成
    ...
    func highlight(model: EqualTaskModel)
    /// 过期语音消息，服务器缓存时间为7天
...
class EngineDataDelegate: NSObject {
...
extension EngineDataDelegate {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func nearAdd(_ tasksArray: [EqualTaskModel]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.addView()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(kStr_normalData) + String(kStr_leadingRangeData.suffix(2)))) {
                //: model.zipFilePath = "\(CacheReactiveCompatible.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(CacheReactiveCompatible.cover())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(kStr_toData.prefix(5)) + kStr_pathValue.replacingOccurrences(of: "read", with: "e") + String(kStr_infoName))) {
                //: model.zipFilePath = "\(CacheReactiveCompatible.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(CacheReactiveCompatible.writtenAccount())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            model.taskStatus = .prepare
            //: GoFarReactiveCompatible.db_updateVoiceMsg(msgId: model.taskId, status: "\(DownloadTaskStatus.prepare.rawValue)", filePath: model.zipFilePath)
            GoFarReactiveCompatible.sendRegister(msgId: model.taskId, status: "\(GiftWithOnsetTarget.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func coequal(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = EqualTaskModel.part(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? EqualTaskModel()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(kStr_normalData) + String(kStr_leadingRangeData.suffix(2)))) {
            //: taskModel.zipFilePath = "\(CacheReactiveCompatible.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(CacheReactiveCompatible.cover())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(kStr_toData.prefix(5)) + kStr_pathValue.replacingOccurrences(of: "read", with: "e") + String(kStr_infoName))) {
            //: taskModel.zipFilePath = "\(CacheReactiveCompatible.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(CacheReactiveCompatible.writtenAccount())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension EngineDataDelegate: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = EqualTaskModel.errorPath(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.highlight(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (kStr_shData.replacingOccurrences(of: "title", with: "el") + String(kStr_commentData)) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    taskGift(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.produce(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                taskGift(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.playerTitle(url: task.taskDescription ?? "")
        }
...
extension EngineDataDelegate {
    ...
    func taskGift(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(GiftWithOnsetTarget.pause.rawValue)" : "\(GiftWithOnsetTarget.error.rawValue)"
            //: GoFarReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            GoFarReactiveCompatible.sendRegister(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.tip(model: taskModel)
            }
    ...
    func produce(url: String) {
        ...
        //: case .chatRecord:
        case .chatRecord:
            //: GoFarReactiveCompatible.db_deleteVoiceMsg(msgId: taskModel.taskId)
            GoFarReactiveCompatible.voiceAppear(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.waitModel(model: taskModel)
    ...
    func playerTitle(url: String) {
        ...
        let size = EqualTaskModel.errorPath(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            //: if taskModel.taskTypeId == .chatRecord {
            if taskModel.taskTypeId == .chatRecord {
                //: GoFarReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: "\(DownloadTaskStatus.finish.rawValue)")
                GoFarReactiveCompatible.sendRegister(msgId: taskModel.taskId, status: "\(GiftWithOnsetTarget.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(kStr_itemViewText.suffix(4)))
            ...
            self.noIndex(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.creature(model: taskModel)
            ...
            //: case .chatRecord:
            case .chatRecord:
                //: GoFarReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: "\(DownloadTaskStatus.expired.rawValue)")
                GoFarReactiveCompatible.sendRegister(msgId: taskModel.taskId, status: "\(GiftWithOnsetTarget.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.equalCommunicationExpiredMTheory(model: taskModel)
    ...
    func noIndex(at path: String, withKeyId keyId: String, withTypeId typeId: FromSucceedAdditiveArithmetic, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(kStr_itemViewText.suffix(4)))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.superpower()
                    ...
                    if typeId == .chatRecord {
                        //: GoFarReactiveCompatible.db_updateVoiceMsg(msgId: keyId, filePath: newPath)
                        GoFarReactiveCompatible.sendRegister(msgId: keyId, filePath: newPath)
                    }
