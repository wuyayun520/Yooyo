...
fileprivate let kStr_pathData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_miss" :*/
fileprivate let kStr_textContent:String = "meco"
fileprivate let kStr_keyName:[Character] = ["n","_","c","h","a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let kStr_modeData:String = "icon_push play selected birthday"
fileprivate let kStr_endSmallName:[Character] = ["c","h"]
fileprivate let kStr_disappearData:String = "name add make equalat_yes"
...
class ProtectionThen: CollectionViewCell {
//    var transBlock: (() -> Void)?

    //: var textData: ShowCellData?
    var textData: ShowCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_pathData.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        //: label.numberOfLines = 0
        label.numberOfLines = 0
        //: label.font = UIFont.messageDown(fontSize: 16)
        label.font = UIFont.messageDown(fontSize: 16)
        //: return label
        return label
        //: }()
...
//        btn.setImage(UIImage.bundleBy(name: "icon_chat_translate_nor"), for: .normal)
//        btn.setImage(UIImage.bundleBy(name: "icon_chat_translate_pre"), for: .highlighted)
...
extension ProtectionThen {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)

        //: if let temTextData = data as? ShowCellData {
        if let temTextData = data as? ShowCellData {
            ...
            if imgPreview.count > 0, temTextData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
                ...
                giftImagV.timeFinish(urlStr: imgPreview)
            }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != TableMacroDefine.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.to() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transShowView.stopAnimating()
                //: self.transStatusImagV.image = UIImage.bundleBy(name: "icon_chat_miss")
                self.transStatusImagV.image = UIImage.bundleBy(name: (kStr_textContent.replacingOccurrences(of: "me", with: "i") + String(kStr_keyName)))
                //: } else if self.textData?.translateStatus == .MsgTranslate_succeed {
            } else if self.textData?.translateStatus == .MsgTranslate_succeed {
                //: self.transShowView.stopAnimating()
                self.transShowView.stopAnimating()
                //: self.transStatusImagV.image = UIImage.bundleBy(name: "icon_chat_yes")
                self.transStatusImagV.image = UIImage.bundleBy(name: (String(kStr_modeData.prefix(5)) + String(kStr_endSmallName) + String(kStr_disappearData.suffix(6))))
            }
    ...
    func theChange() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(touchSend(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == TableMacroDefine.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == TableMacroDefine.toVoice())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            bigness()
