...
fileprivate let kStr_miniData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "[未知消息]" :*/
fileprivate let kStr_lineValue:String = "[未知消息]"

/*: "extra" :*/
fileprivate let kStr_blockData:[Character] = ["e","x","t","r","a"]

/*: "</at> " :*/
fileprivate let kStr_existData:String = "</at> model load"

/*: "#0084FF" :*/
fileprivate let kStr_data:[Character] = ["#","0","0","8","4","F","F"]

/*: "#FFEE4B" :*/
fileprivate let kStr_imageData:String = "user of#FFEE"
fileprivate let kStr_sizeName:String = "4Bview height push label"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let kStr_tableValue:[Character] = ["<","a","t",">","@","[","\\","S","\\","s","]","+","?","<","/","a","t",">"]

/*: "jumpKey" :*/
fileprivate let kStr_lineData:String = "jumpKeynot platform extra interval add"

/*: "user" :*/
fileprivate let kStr_directionValue:String = "USER"

/*: "uid" :*/
fileprivate let kStr_targetData:[UInt8] = [0xb5,0xa9,0xa4]

private func sexAdd(last num: UInt8) -> UInt8 {
    return num ^ 192
}

/*: "#16D073" :*/
fileprivate let kStr_closeCenterText:[Character] = ["#","1","6","D","0","7","3"]

/*: "#F95644" :*/
fileprivate let kStr_itemMomentValue:String = "status any kind top#F95644"
...
@objcMembers public class CollectionThen: TalkingLoadCollectionThen {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_miniData.reversed(), encoding: .utf8)!)
    }
    ...
    override public var msgModel: KeyModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (kStr_lineValue.capitalized)
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = doInfo(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func doInfo(msgInfoModel: LabelOffAimChromosphereJsonModel) -> NSMutableAttributedString {
        ...
        guard dic[(String(kStr_blockData))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.dataFormat() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(kStr_data)))! : UIColor(hex: (String(kStr_imageData.suffix(5)) + String(kStr_sizeName.prefix(2))))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(kStr_tableValue)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo["jumpKey"] == "user" {
                ...
                uid = jumpInfo[String(bytes: kStr_targetData.map{sexAdd(last: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: kStr_targetData.map{sexAdd(last: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func countlessness() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(kStr_closeCenterText)))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(kStr_itemMomentValue.suffix(7))))!
        }
