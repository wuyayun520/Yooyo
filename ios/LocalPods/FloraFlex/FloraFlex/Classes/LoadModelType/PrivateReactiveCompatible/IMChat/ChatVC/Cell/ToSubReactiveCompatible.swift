...
fileprivate let kStr_tableTitle:[UInt8] = [0xfa,0xfd,0xfa,0xe7,0xbb,0xf0,0xfc,0xf7,0xf6,0xe1,0xa9,0xba,0xb3,0xfb,0xf2,0xe0,0xb3,0xfd,0xfc,0xe7,0xb3,0xf1,0xf6,0xf6,0xfd,0xb3,0xfa,0xfe,0xe3,0xff,0xf6,0xfe,0xf6,0xfd,0xe7,0xf6,0xf7]

private func appContext(center num: UInt8) -> UInt8 {
    return num ^ 147
}

/*: "icon_chat_start" :*/
fileprivate let kStr_valueAgeInsertData:[Character] = ["i","c","o","n","_","c","h"]
fileprivate let kStr_areaCoverValue:String = "at_startkit name any"

/*: "icon_chat_lock" :*/
fileprivate let kStr_rootEqualData:String = "block frame foricon_chat"
fileprivate let kStr_requestName:[Character] = ["_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let kStr_itemFlowName:[Character] = ["i","c","o","n","_","c","h","a","t"]
fileprivate let kStr_equalData:[Character] = ["_","b","u","r","n","e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let kStr_audienceTitleData:String = "icbuttonn"
fileprivate let kStr_cancelYouData:String = "bnamerne"

/*: "Already burned" :*/
fileprivate let kStr_toShapeValue:String = "Alrapp bottom income"
fileprivate let kStr_managerData:String = "ecommondy"
...
class ToSubReactiveCompatible: CollectionViewCell {
    //: var videoData: CountCellData?
    var videoData: CountCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_tableTitle.map{appContext(center: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        //: imgV.contentMode = .scaleAspectFill
        imgV.contentMode = .scaleAspectFill
        //: imgV.image = UIImage.bundleBy(name: "icon_chat_start")
        imgV.image = UIImage.bundleBy(name: (String(kStr_valueAgeInsertData) + String(kStr_areaCoverValue.prefix(8))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        //: imgV.contentMode = .scaleAspectFill
        imgV.contentMode = .scaleAspectFill
        //: imgV.image = UIImage.bundleBy(name: "icon_chat_lock")
        imgV.image = UIImage.bundleBy(name: (String(kStr_rootEqualData.suffix(9)) + String(kStr_requestName)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: OverdoTalkingButton = {
        ...
        imgV.setBackgroundImage(UIImage.bundleBy(name: (String(kStr_itemFlowName) + String(kStr_equalData))), for: .normal)
        //: imgV.setImage(UIImage.bundleBy(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.bundleBy(name: (kStr_audienceTitleData.replacingOccurrences(of: "button", with: "o") + "_chat_" + kStr_cancelYouData.replacingOccurrences(of: "name", with: "u") + "d_shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(kStr_toShapeValue.prefix(3)) + kStr_managerData.replacingOccurrences(of: "common", with: "a") + " burned").localized, for: .normal)
...
extension ToSubReactiveCompatible {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: self.videoData = data as? CountCellData
        self.videoData = data as? CountCellData
        ...
        self.dutationLab.text = NSDate.belowTheme(totalTime: duration)
        ...
        self.form(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func form(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.primaryLevel(image, withFile: 90)
                self.picImgView.image = UIImage.primaryLevel(image, withFile: 90)
                //: } else {
            } else {
            ...
            self.picImgView.playingCard(urlStr: urlStr) { [weak self] image in
