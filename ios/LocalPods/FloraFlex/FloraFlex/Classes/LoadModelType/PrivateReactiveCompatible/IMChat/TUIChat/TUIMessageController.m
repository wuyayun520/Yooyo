...
- (void)setupViews {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(menuWillHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNewMessage:) name:[LoadNotificationTool outpostNoti] object:nil];
...
- (void)loadMessage {
    ...
    if (self.currentChatType == TalkingIMChatType_group ||
        ...
        [[V2TIMManager sharedInstance] getGroupHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([LayerConversationListener shared].isConnection) { //推送点进来会报错
                NSString *str = [@"Net Error, Try again later" imageShow];
                [self detailDownArrayWithShowMsg:[str imageShow]];
            }
        ...
        [[V2TIMManager sharedInstance] getC2CHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([LayerConversationListener shared].isConnection) {                
                if (LocationThen.share.appStatus == 0) {
                    NSString *str = [@"Net Error, Try again later" imageShow];
                    [self detailDownArrayWithShowMsg:[str imageShow]];
                }
            }
...
- (void)getMessages:(NSArray *)msgs msgCount:(int)msgCount {
    ...
    for (NSInteger k = array.count - 1; k >= 0; --k) {
        ...
        if ([userId isEqualToString:LocationThen.share.loginUserMode.userID]) {
...
- (void)onNewMessage:(NSNotification *)notification
{
    ...
    if ([msg.sender isEqualToString:LocationThen.share.loginUserMode.userID]) {
        //过滤本人发送的消息
        return;
    }
    ...
    if (uiMsgs.count > 0) {
        ...
        if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [TableMacroDefine appearSumeraction] * 1.5) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    缓存存在问题待处理
    //    if (height > 0 && ![data isKindOfClass:[AbTimeCellData class]] && ![data isKindOfClass:[SelectCellData class]]) {
    //        return height;
    //    }
...
- (void)sendCustomMessage:(TUIMessageCellData *)msg {
    ...
    NSInteger sender = LocationThen.share.loginUserMode.userID.integerValue;
    NSString *msgID;
    if (isGroup) {
...
- (void)modifyCustomMessageDataWithExtral:(NSDictionary *)extral cellData:(TUIMessageCellData *)msg {
    ...
    // 更新内存模型
    msg.msgModel = [KeyModel modelWithDic:newExtraDict];
}
...
///   - msg: 图片消息
///   - model: 私密照片模型
- (void)changeIntimatePhotoMsg:(TUIMessageCellData *)msg photoModel:(AddModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [AddModel image:model contiguous:msg.msgID];
...
///   - msg: 视频消息
///   - model: 私密视频模型
- (void)changeIntimateVideoMsg:(TUIMessageCellData *)msg videoModel:(ViewJsonModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [ViewJsonModel image:model effect:msg.msgID];
...
- (void)changeIntimateGiftMsg:(TUIMessageCellData *)msg {
    dispatch_async(dispatch_get_main_queue(), ^{
        [EmptyContentModel rescueCreate:msg.msgID];
...
- (TalkingSystemMsgCellData *)transSystemMsgFromDate:(NSDate *)date
{
    if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > MAX_MESSAGE_SEP_DLAY) {
        ...
        system.contentStr = [NSDate teachingMethodDateWithShowDetail:YES date:date];
...
- (void)onLongPressMessageWithCell:(TUIMessageCell *)cell{
    ...
    BOOL hasJumps = NO;
    if ([data isKindOfClass:[ShowCellData class]]) {
        ShowCellData *textCellData = (ShowCellData *)data;
    ...
    // 只有文字才有复制功能
    if (([data isKindOfClass:[CollectionThen class]] ||
         [data isKindOfClass:[ShowCellData class]]) && hasJumps == NO) {
        // 过滤掉音视频通话文本消息
        if (data.msgModel.msgInfo.callCmd == nil) {
            [items addObject:[[UIMenuItem alloc] initWithTitle:@"Copy".imageShow action:@selector(onCopyMsg:)]];
            // 私聊 && 接收消息 && 消息未翻译成功
            if ([data isKindOfClass:[ShowCellData class]] &&
                data.direction == MsgDirectionIncoming &&
                data.translateStatus != MsgTranslate_succeed) {
                [items addObject:[[UIMenuItem alloc] initWithTitle:@"Trans".imageShow action:@selector(onTranslateMsg:)]];
            }
        }
    }
    
    if ([self canReplyResponseWithLongGestureRecognizer:data]) {
        [items addObject:[[UIMenuItem alloc] initWithTitle:@"Reply".imageShow action:@selector(onReplyMsg:)]];
    }
...
- (NSArray *)requiredResponseWithLongGestureRecognizer {
    return @[[CollectionThen class],
             [ShowCellData class],
             [CanPointCellData class],
             [LayerCellData class],
             [CountCellData class],
             [FillCellData class],
             [LoadCellData class]];
...
- (void)onCopyMsg:(id)sender {
    ...
    if ([_menuUIMsg isKindOfClass:[CollectionThen class]] ||
        [_menuUIMsg isKindOfClass:[ShowCellData class]]) {
        ShowCellData *txtMsg = (ShowCellData *)_menuUIMsg;
...
- (BOOL)canReplyResponseWithLongGestureRecognizer:(TUIMessageCellData *)cellData {
    ...
    if ([cellData isKindOfClass:[FillCellData class]] || [cellData isKindOfClass:[CanPointCellData class]]) {
        return YES;
    }
...
- (void)translateMsgWithCellData:(TUIMessageCellData *)cellData {
    ...
    if (LocationThen.share.appUserConfigMode.enableTranslate == NO) { // 非会员，无法使用翻译
        [[EffectPushManager share] verso];
        return;
    }
    
    if ([cellData isKindOfClass:[ShowCellData class]]) {
        ShowCellData *txtMsg = (ShowCellData *)cellData;
        [self translateMessageToReq:txtMsg andType:0];
        
    } else if ([cellData isKindOfClass:[CollectionThen class]]) {
        CollectionThen *txtMsg = (CollectionThen *)cellData;
...
- (void)translateMessageToReq:(TUIMessageCellData *)txtMsg andType:(int)type {
    ...
    // 调用翻译接口
    [UpsetCountRequestTool colorWithCompletionWithMsgId:txtMsg.msgModel.msgInfo.msgId type:type completion:^(BOOL succeed, NSDictionary *dict, NSInteger errorCode) {
        if (!succeed) {
            if (errorCode == 403) { // 非会员，无法使用翻译
                LocationThen.share.appUserConfigMode.enableTranslate = NO;
                [[EffectPushManager share] verso];
            }
