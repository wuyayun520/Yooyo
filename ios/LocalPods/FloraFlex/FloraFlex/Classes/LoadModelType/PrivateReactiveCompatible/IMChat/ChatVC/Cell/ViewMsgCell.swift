...
fileprivate let kStr_takeText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_translate_nor" :*/
fileprivate let kStr_titleValue:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r","a","n","s","l","a"]
fileprivate let kStr_modelName:[Character] = ["t","e","_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let kStr_imageName:String = "ICON"
fileprivate let kStr_downBringData:String = "_tranview to draft first name"
fileprivate let kStr_contentName:[Character] = ["e"]

/*: "icon_chat_miss" :*/
fileprivate let kStr_dataText:[Character] = ["i","c","o","n","_","c","h","a","t","_","m"]
fileprivate let kStr_domainData:String = "ipointpoint"

/*: "icon_chat_yes" :*/
fileprivate let kStr_valueTitle:[Character] = ["i","c","o","n","_"]
fileprivate let kStr_transactionActionName:[Character] = ["c","h"]
fileprivate let kStr_labelFewNameTitle:[Character] = ["a","t","_","y","e","s"]
...
class ViewMsgCell: CollectionViewCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: CollectionThen?
    var textData: CollectionThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_takeText.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.bundleBy(name: (String(kStr_titleValue) + String(kStr_modelName))), for: .normal)
        //: btn.setImage(UIImage.bundleBy(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.bundleBy(name: (kStr_imageName.lowercased() + "_chat" + String(kStr_downBringData.prefix(5)) + "slate_pr" + String(kStr_contentName))), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(cellMode), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ViewMsgCell {
    ...
    override func fill(with data: TCommonCellData) {
        ...
        super.fill(with: data)
        //: textData = data as? CollectionThen
        textData = data as? CollectionThen
        ...
        //: let maxWidth = TableMacroDefine.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = TableMacroDefine.equalWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.bundleBy(name: (String(kStr_dataText) + kStr_domainData.replacingOccurrences(of: "point", with: "s")))
            //: } else if textData.translateStatus == .MsgTranslate_succeed {
        } else if textData.translateStatus == .MsgTranslate_succeed {
            //: self.transShowView.stopAnimating()
            self.transShowView.stopAnimating()
            //: self.transStatusImagV.image = UIImage.bundleBy(name: "icon_chat_yes")
            self.transStatusImagV.image = UIImage.bundleBy(name: (String(kStr_valueTitle) + String(kStr_transactionActionName) + String(kStr_labelFewNameTitle)))
        }
    ...
    func endless() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(sourceModel(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func sourceModel(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
