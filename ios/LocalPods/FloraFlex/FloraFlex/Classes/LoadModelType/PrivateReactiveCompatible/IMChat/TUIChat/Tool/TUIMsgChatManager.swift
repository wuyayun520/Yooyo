...
//
//        let userDic = ["uid":LocationThen.share.loginUserMode.userID,"nickName":LocationThen.share.loginUserMode.nickname,"headPic":LocationThen.share.loginUserMode.headPic,"mbId":msgInfo["mbId"]]
...
//
//        if msgTypeStr == "gift" {
//            let cellData = AbFillCellData.init(direction: (isSend ? TMsgDirection.MsgDirectionOutgoing : TMsgDirection.MsgDirectionIncoming))
//            let data = extraString.data(using: .utf8)
//            cellData.innerMessage = V2TIMManager.sharedInstance().createCustomMessage(data)
//            do {
//                let msgModel = try KeyModel.init(dictionary: tempDic)
...
//
//            let cellData = ShowCellData.init(direction: (isSend ? TMsgDirection.MsgDirectionOutgoing : TMsgDirection.MsgDirectionIncoming))
//            let data = extraString.data(using: .utf8)
//            cellData.innerMessage = V2TIMManager.sharedInstance().createCustomMessage(data)
//            do {
//                let msgModel = try KeyModel.init(dictionary: tempDic)
...
//            let cellData = LayerCellData.init(direction: (isSend ? TMsgDirection.MsgDirectionOutgoing : TMsgDirection.MsgDirectionIncoming))
//            let data = extraString.data(using: .utf8)
//            cellData.innerMessage = V2TIMManager.sharedInstance().createCustomMessage(data)
//            do {
//                let msgModel = try KeyModel.init(dictionary: tempDic)
...
//            let cellData = CanPointCellData.init(direction: (isSend ? TMsgDirection.MsgDirectionOutgoing : TMsgDirection.MsgDirectionIncoming))
//            let data = extraString.data(using: .utf8)
//            cellData.innerMessage = V2TIMManager.sharedInstance().createCustomMessage(data)
//            do {
//                let msgModel = try KeyModel.init(dictionary: tempDic)
...
extension TUIMsgChatManager {
    ...
    class func checkVoiceMessage(message: V2TIMMessage) -> Bool {
        ...
        guard let extra = String(data: message.customElem.data, encoding: .utf8) else {
            pathLine(message: " customElem.data is error")
            return false
        }
