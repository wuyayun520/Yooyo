...
fileprivate let kStr_cellData:[UInt8] = [0x9d,0xa2,0x9d,0xa8,0x5c,0x97,0xa3,0x98,0x99,0xa6,0x6e,0x5d,0x54,0x9c,0x95,0xa7,0x54,0xa2,0xa3,0xa8,0x54,0x96,0x99,0x99,0xa2,0x54,0x9d,0xa1,0xa4,0xa0,0x99,0xa1,0x99,0xa2,0xa8,0x99,0x98]

fileprivate func whiteHide(normal num: UInt8) -> UInt8 {
...
/*: "nav_back_black_nor" :*/
fileprivate let kStr_viewContent:[Character] = ["n","a","v","_","b","a","c","k","_","b","l"]
fileprivate let kStr_shadowData:[Character] = ["a","c","k","_","n","o","r"]

/*: "btn_detail_more" :*/
fileprivate let kStr_atProgressValue:String = "to user size gesturebtn_de"
fileprivate let kStr_effectValue:String = "ORE"

/*: "btn_chat_videocall" :*/
fileprivate let kStr_timeSubValue:String = "btn_creturn background"
fileprivate let kStr_mainData:String = "breathingdeo"
fileprivate let kStr_makeValue:[Character] = ["c","a","l","l"]

/*: "icon_lounge" :*/
fileprivate let kStr_numberTextTitle:String = "system equalicon_"

/*: "Official" :*/
fileprivate let kStr_equalData:[Character] = ["O","f","f","i","c","i","a","l"]

/*: "Online List" :*/
fileprivate let kStr_makeName:[Character] = ["O","n","l","i","n","e"," ","L","i","s","t"]

/*: "icon_privateChat_intimate" :*/
fileprivate let kStr_ofBackData:[Character] = ["i","c","o","n","_","p","r","i","v","a"]
fileprivate let kStr_modelName:String = "teChedge premium"
fileprivate let kStr_indexContent:String = "top color to cell viewat_int"

/*: "#FF60B9" :*/
fileprivate let kStr_itemText:[Character] = ["#","F","F","6","0","B","9"]

/*: "Customer Care Center" :*/
fileprivate let kStr_actionValue:String = "mode height name hide noneCust"
fileprivate let kStr_resultName:String = "user letare Cente"
fileprivate let kStr_objectValue:[Character] = ["r"]

/*: "#48FF1E" :*/
fileprivate let kStr_managerValue:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let kStr_matchStyleName:String = "size model status video fromOnline"

/*: "#FF4B7F" :*/
fileprivate let kStr_atContent:String = "#FF4B7Fmodel content of normal all"

/*: "busy" :*/
fileprivate let kStr_userName:String = "BUSY"

/*: "+ :*/
fileprivate let kStr_languageValue:String = "frame"

/*: "get json error" :*/
fileprivate let kStr_aspectName:String = "get jsotransition var height"
fileprivate let kStr_permissionData:String = "n errorsize ting view"

/*: "transform.rotation" :*/
fileprivate let kStr_cellContent:[Character] = ["t","r","a","n","s","f","o"]
fileprivate let kStr_showEmptyValue:String = "valuem"
fileprivate let kStr_searchText:String = ".rotaview model"

/*: "transform.scale" :*/
fileprivate let kStr_toParaPrivateTitle:[Character] = ["t","r","a","n","s","f","o","r","m",".","s","c"]
fileprivate let kStr_voiceTitle:String = "withle"

/*: "zoom&shake" :*/
fileprivate let kStr_cellName:[Character] = ["z","o","o","m","&","s","h","a","k","e"]
...
class GiftThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_cellData.map{whiteHide(normal: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var backButton: UIButton = {
        ...
        let img = UIImage.bundleBy(name: (String(kStr_viewContent) + String(kStr_shadowData))).withRenderingMode(.alwaysTemplate)
    ...
    lazy var moreButton: UIButton = {
        ...
        let img = UIImage.bundleBy(name: (String(kStr_atProgressValue.suffix(6)) + "tail_m" + kStr_effectValue.lowercased())).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
    ...
    lazy var videoCallBtn: UIButton = {
        //: let btn = UIButton(type: .custom)
        let btn = UIButton(type: .custom)
        //: let img = UIImage.bundleBy(name: "btn_chat_videocall").withRenderingMode(.alwaysTemplate)
        let img = UIImage.bundleBy(name: (String(kStr_timeSubValue.prefix(5)) + "hat_v" + kStr_mainData.replacingOccurrences(of: "breathing", with: "i") + String(kStr_makeValue))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
    ...
    lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.bundleBy(name: (String(kStr_numberTextTitle.suffix(5)) + "lounge"))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(kStr_equalData)).localized, for: .normal)
    ...
    lazy var onlineListBtn: UIButton = {
        ...
        btn.setTitle((String(kStr_makeName)).localized, for: .normal)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineStatusV: OverdoTalkingButton = {
    ...
    private lazy var myBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(pathTarget), for: .touchUpInside)
    ...
    private lazy var intimateBtn: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.bundleBy(name: "icon_privateChat_intimate"), for: .normal)
        btn.setImage(UIImage.bundleBy(name: (String(kStr_ofBackData) + String(kStr_modelName.prefix(4)) + String(kStr_indexContent.suffix(6)) + "imate")), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(withMagnitudeimate), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var intimateLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (String(kStr_itemText)))
    ...
    private lazy var addIntimateLB: UILabel = {
        ...
        lb.font = UIFont.groupSize(type: .Semibold, fontSize: 12)
        //: lb.isHidden = true
        lb.isHidden = true
...
extension GiftThen {
    ...
    func tab() {
        ...
        if self.chatType == .service {
            ...
            titleLabel.text = (String(kStr_actionValue.suffix(4)) + "omer C" + String(kStr_resultName.suffix(9)) + String(kStr_objectValue)).localized
            //: titleLabel.font = .pingfangFont(type: .Medium, fontSize: 18)
            titleLabel.font = .groupSize(type: .Medium, fontSize: 18)
            //: titleLabel.textColor = .appTitleColor()
            titleLabel.textColor = .dataFormat()
    ...
    func resembleConstraints() {
        ...
        if self.chatType == .service { // 官方客服
            ...
            let offset = DeleteClientThen.shared.direction == .leftToRight ? 22 : -22
    ...
    ///   - userModel: 用户信息
    ///   - isFrom: 列表控制器入口
    //: func private_updateNavView(userModel: FailureReactiveCompatible, isFrom: PrivateChatIsFromEnum) {
    func magnitudeeraction(userModel: FailureReactiveCompatible, isFrom: ClientFromEnum) {
        ...
        if userModel.onlineStatus == 1 {
            ...
            let image = UIImage.untilSkinColour(color: UIColor(hex: (String(kStr_managerValue)))!, size: CGSize(width: 8, height: 8))
            //: onlineStatusV.setImage(image.imageCircle(), for: .normal)
            onlineStatusV.setImage(image.imageCircle(), for: .normal)
            //: onlineStatusV.setTitle("Online".localized, for: .normal)
            onlineStatusV.setTitle((String(kStr_matchStyleName.suffix(6))).localized, for: .normal)
            ...
            let image = UIImage.untilSkinColour(color: UIColor(hex: (String(kStr_atContent.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineStatusV.setImage(image.imageCircle(), for: .normal)
            onlineStatusV.setImage(image.imageCircle(), for: .normal)
            //: onlineStatusV.setTitle("busy".localized, for: .normal)
            onlineStatusV.setTitle((kStr_userName.lowercased()).localized, for: .normal)
            //: } else {
        } else {
        ...
        titleLabel.textColor = userModel.loungePlus ? .makeColor() : .white
        ...
        var videoCall = userModel.enableVideoCall
        //: if isFrom == .LiveRoom || LocationThen.share.appStatus == AppSkinStatus.special.rawValue {
        if isFrom == .LiveRoom || LocationThen.share.appStatus == SharedInsetTarget.special.rawValue {
            //: videoCall = false
            videoCall = false
        }
        //: if videoCall {
    ...
    /// - Parameters:
    ///   - userModel: 用户信息
    //: func private_updateIntimateNavView(userModel: FailureReactiveCompatible) {
    func restorePush(userModel: FailureReactiveCompatible) {
        ...
        userBtn.isHidden = false
        //: myBtn.setUrlImage(urlStr: LocationThen.share.loginUserMode.headPic ?? "")
        myBtn.displayFinish(urlStr: LocationThen.share.loginUserMode.headPic ?? "")
        //: userBtn.setUrlImage(urlStr: userModel.headPic)
        userBtn.displayFinish(urlStr: userModel.headPic)
    ...
    func indexCountimate(intimate: Int) {
        //: let str  = "+\(intimate)"
        let str = "+\(intimate)"
        ...
        let attributes = [.foregroundColor: UIColor.white, .font: UIFont.groupSize(type: .Semibold, fontSize: 12)] as [NSAttributedString.Key: Any]
        ...
        let url = LoadGrappleEffectTool.default.sunnahType(type: .Private_Chat_intimate)
        //: do {
        do {
            ...
            pathLine(message: (String(kStr_aspectName.prefix(7)) + String(kStr_permissionData.prefix(7))))
        }
    }
    ...
    func screenCountimate(intimate: Int) {
        //: intimateLB.text = "\(intimate)"
        intimateLB.text = "\(intimate)"
    }

    //: @objc func myBtnClick() {
    @objc func pathTarget() {
        //: EffectPushManager.share.func__pushToUserDetailVC(uid: LocationThen.share.loginUserMode.userID)
        EffectPushManager.share.userAdd(uid: LocationThen.share.loginUserMode.userID)
    }

    //: @objc func userIconClick() {
    @objc func iconAction() {
        //: EffectPushManager.share.func__pushToUserDetailVC(uid: userId)
        EffectPushManager.share.userAdd(uid: userId)
    }

    //: @objc func intimateBtnClick() {
    @objc func withMagnitudeimate() {
        //: let view = TalkingIntimateInstructionsPopUpView.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight))
        let view = EraseViewDelegate(frame: CGRect(x: 0, y: 0, width: kLet_halfData, height: kLet_noSeatName))
        //: view.show()
        view.dataGift()
    }
    ...
    func animatShow() {
        ...
        let roate = CAKeyframeAnimation(keyPath: (String(kStr_cellContent) + kStr_showEmptyValue.replacingOccurrences(of: "value", with: "r") + String(kStr_searchText.prefix(5)) + "tion"))
        ...
        let zoom = CAKeyframeAnimation(keyPath: (String(kStr_toParaPrivateTitle) + kStr_voiceTitle.replacingOccurrences(of: "with", with: "a")))
        ...
        videoCallBtn.layer.add(groupAnnimation, forKey: (String(kStr_cellName)))
    }
}
