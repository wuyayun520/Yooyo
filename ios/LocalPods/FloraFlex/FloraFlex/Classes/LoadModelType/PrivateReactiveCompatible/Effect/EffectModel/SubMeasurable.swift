...
/*: "id" :*/
fileprivate let kStr_playerText:String = "ipackage"

/*: "num" :*/
fileprivate let kStr_blockCenterValue:[Character] = ["n","u","m"]
...
@objc public enum ClientValueWidthCount: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct SubMeasurable: HandyJSON {
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// ClientValueWidthCount
    ...
    mutating func setUponLagniappe(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct AscertainModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(kStr_blockCenterValue))
    }
}
