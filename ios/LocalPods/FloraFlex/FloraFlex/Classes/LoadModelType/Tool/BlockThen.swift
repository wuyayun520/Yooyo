...
/*: "gifts" :*/
fileprivate let kStr_toName:[Character] = ["g","i","f","t","s"]

/*: "vipGifts" :*/
fileprivate let kStr_giftTouchData:String = "vipGiftframe gift text let center"
fileprivate let kStr_seatName:String = "S"

/*: "packGifts" :*/
fileprivate let kStr_keyData:[Character] = ["p","a","c","k","G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let kStr_imageOnTitle:String = "giftSendmax true"
fileprivate let kStr_errorName:String = "Itemview scene make"
...
class BlockThen: NSObject {
    ...
    private var cacheDict = [String: [AddColumnConvertible: [Any]]]()
    // 数据锁
    ...
    private var giftType = CanScalarLiteral.normal

    //: static let share = TalkingChatGiftManager()
    static let share = BlockThen()
...
extension BlockThen {
    ...
    func sectionDismiss(type: CanScalarLiteral = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        BlockThen.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[AddColumnConvertible.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            BlockThen.share.filterWith(completion: { isSucceed in
            ...
            BlockThen.share.stud(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            BlockThen.share.detailEqual(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            BlockThen.share.countEqual(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func countEqual(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = BlockThen.share.giftType
        //: UpsetCountRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        UpsetCountRequestTool.systemObject(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [AddColumnConvertible: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(String(kStr_toName))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<EngineHandyJSON>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[AddColumnConvertible.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(kStr_giftTouchData.prefix(7)) + kStr_seatName.lowercased())] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<EngineHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[AddColumnConvertible.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(kStr_keyData))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<EngineHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[AddColumnConvertible.Bags] = bagsArr
                }
                ...
                var firstModel = SharedModelType()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(kStr_imageOnTitle.prefix(8)) + String(kStr_errorName.prefix(4)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<SharedModelType>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func filterWith(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = BlockThen.share.giftType
        //: UpsetCountRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        UpsetCountRequestTool.withPresent(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func companyTitle(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: UpsetCountRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        UpsetCountRequestTool.membersCompletion(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<BottomMemberModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func stud(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = CanScalarLiteral.live
        //: UpsetCountRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        UpsetCountRequestTool.bite(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func detailEqual(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = CanScalarLiteral.party
        //: UpsetCountRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        UpsetCountRequestTool.liberalParty(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func plus(perPageCount: Int, dataType: AddColumnConvertible) -> Int {
    ...
    func turnType(dataType: AddColumnConvertible) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func mic(dataType: AddColumnConvertible, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension BlockThen {
    ...
    @objc private func acceptButtonSheaf() {
        //: gift_clearMemoryCache(showRed: true)
        bestowmentAddRubyRed(showRed: true)
    }
}
...
extension BlockThen {
    ...
    func bestowmentAddRubyRed(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        BlockThen.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
