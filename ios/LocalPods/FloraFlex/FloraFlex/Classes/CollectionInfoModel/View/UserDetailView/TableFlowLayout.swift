...
protocol TicketLayoutDelegate: NSObjectProtocol {
...
class TableFlowLayout: UICollectionViewFlowLayout {
    ...
    var delegate: TicketLayoutDelegate?
    //: var maxIneteritemSpacing: CGFloat = actualWidth(w: 6)
    var maxIneteritemSpacing: CGFloat = originalScene(w: 6)
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for i in 1..<attributes.count {
            ...
            if i == attributes.count-1 {
                ...
                self.delegate?.become(layoutH: curAttr.frame.maxY)
            }
        }
