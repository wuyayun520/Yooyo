...
fileprivate let kStr_addName:[UInt8] = [0x1,0x6,0x1,0xc,0xc0,0xfb,0x7,0xfc,0xfd,0xa,0xd2,0xc1,0xb8,0x0,0xf9,0xb,0xb8,0x6,0x7,0xc,0xb8,0xfa,0xfd,0xfd,0x6,0xb8,0x1,0x5,0x8,0x4,0xfd,0x5,0xfd,0x6,0xc,0xfd,0xfc]

fileprivate func bagSize(number num: UInt8) -> UInt8 {
...
/*: "Birthday" :*/
fileprivate let kStr_sendSharedName:String = "Birthdayif class in in finish"

/*: "btn_me_edit" :*/
fileprivate let kStr_keyName:String = "btn_mein let gender"

/*: - :*/
fileprivate let kStr_photoData:[Character] = ["-"]
...
class DataThen: UITableViewCell, UpVoiceObjectProtocol {
    //: var myDateView: TalkingDateView!
    var myDateView: ClientThen!
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_addName.map{bagSize(number: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(kStr_sendSharedName.prefix(8))).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.bundleBy(name: (String(kStr_keyName.prefix(6)) + "_edit")))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var dataBtn: UIButton = {
        ...
        //: dataBtn.setTitle(LocationThen.share.loginUserMode.birthday, for: .normal)
        dataBtn.setTitle(LocationThen.share.loginUserMode.birthday, for: .normal)
        //: dataBtn.setTitleColor(UIColor.appTitleColor(), for: .normal)
        dataBtn.setTitleColor(UIColor.dataFormat(), for: .normal)
        //: dataBtn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 16)
        dataBtn.titleLabel?.font = UIFont.groupSize(type: .Medium, fontSize: 16)
        //: dataBtn.addTarget(self, action: #selector(dataBtnClick), for: .touchUpInside)
        dataBtn.addTarget(self, action: #selector(giftBlock), for: .touchUpInside)
        //: return dataBtn
        return dataBtn
        //: }()
    }()
...
extension DataThen {
    ...
    @objc func giftBlock() {
        //: EffectPushManager.share.func__getCurrentActivityVC()?.view.endEditing(true)
        EffectPushManager.share.quantityimateTarget()?.view.endEditing(true)
        //: myDateView = TalkingDateView.init()
        myDateView = ClientThen()
        ...
        if parts?.count ?? 0 >= 3 {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                ...
                self.myDateView.tablePlaceUpright(year: year, month: month, day: day)
            }
        }
    }
