...
fileprivate let kStr_normalValue:[UInt8] = [0x17,0x1c,0x17,0x22,0xd6,0x11,0x1d,0x12,0x13,0x20,0xe8,0xd7,0xce,0x16,0xf,0x21,0xce,0x1c,0x1d,0x22,0xce,0x10,0x13,0x13,0x1c,0xce,0x17,0x1b,0x1e,0x1a,0x13,0x1b,0x13,0x1c,0x22,0x13,0x12]

fileprivate func cornerMake(model num: UInt8) -> UInt8 {
...
/*: "Post" :*/
fileprivate let kStr_firstStyleData:String = "with style toPost"

/*: "Profile" :*/
fileprivate let kStr_awakeTitleName:[Character] = ["P","r","o","f","i","l","e"]

/*: "bth_me_follow_nor" :*/
fileprivate let kStr_modelValue:[Character] = ["b","t","h","_","m"]
fileprivate let kStr_freshValue:[Character] = ["e","_"]
fileprivate let kStr_skipText:String = "fovaluevalue"

/*: "party_mini_bg_userDetail" :*/
fileprivate let kStr_showData:String = "pabottom"
fileprivate let kStr_pictureTitle:String = "ni_bg_label view to view count"
fileprivate let kStr_managerContent:String = "normalail"

/*: "removeAttentionUid" :*/
fileprivate let kStr_frameUserValue:[Character] = ["r","e","m","o","v","e","A"]
fileprivate let kStr_sendContent:String = "ttcontextnti"

/*: "attentionUid" :*/
fileprivate let kStr_dataValue:[Character] = ["a","t","t","e","n","t","i","o","n","U","i","d"]

/*: "bth_me_follow_pre" :*/
fileprivate let kStr_gameAddName:[UInt8] = [0xd7,0xe9,0xdd,0xd4,0xe2,0xda,0xd4,0xdb,0xe4,0xe1,0xe1,0xe4,0xec,0xd4,0xe5,0xe7,0xda]

fileprivate func makeFalse(the num: UInt8) -> UInt8 {
...
class BackgroundHeightModel: NSObject {
...
class ClientShowViewDelegate: LayerRecognizerDelegate {
    //: var uid: String = ""
    var uid: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_normalValue.map{cornerMake(model: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var navView: PathView = { // 导航
    ...
    private lazy var headerView: EngineManagerView = { // banner
        ...
        let v = EngineManagerView(frame: frame)
    ...
    private lazy var bottomView: TopperBottomView = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = TopperBottomView(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: kLet_postPartyValue, width: kLet_halfData, height: 44))
        ...
        segmentedDataSource.isSelectedAnimable = false
        //: segmentedDataSource.titleNormalColor = UIColor.statusMin()
        segmentedDataSource.titleNormalColor = UIColor.statusMin()
        //: segmentedDataSource.titleSelectedColor = UIColor.appTitleColor()
        segmentedDataSource.titleSelectedColor = UIColor.dataFormat()
        ...
        indicator.indicatorColor = UIColor.dataFormat()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(kLet_netName)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(kStr_firstStyleData.suffix(4))).localized, (String(kStr_awakeTitleName)).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [LayerRecognizerDelegate] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: LocationViewController = {
    ...
    private lazy var profileVC: EqualViewController = {
    ...
    private lazy var followBtn: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.bundleBy(name: "bth_me_follow_nor")
        img.image = UIImage.bundleBy(name: (String(kStr_modelValue) + String(kStr_freshValue) + kStr_skipText.replacingOccurrences(of: "value", with: "l") + "ow_nor"))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(followButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(betweenModel))
    ...
    private lazy var voiceRoomBtn: UIButton = {
        ...
        voiceRoomBtn.isHidden = true
        //: voiceRoomBtn.setBackgroundImage(UIImage.bundleBy(name: "party_mini_bg_userDetail"), for: .normal)
        voiceRoomBtn.setBackgroundImage(UIImage.bundleBy(name: (kStr_showData.replacingOccurrences(of: "bottom", with: "rt") + "y_mi" + String(kStr_pictureTitle.prefix(6)) + "userD" + kStr_managerContent.replacingOccurrences(of: "normal", with: "et"))), for: .normal)
        //: return voiceRoomBtn
        return voiceRoomBtn
        //: }()
    }()
...
extension ClientShowViewDelegate {
    /// follow
    //: private func req_followRequest() {
    private func enableAction() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(String(kStr_frameUserValue) + kStr_sendContent.replacingOccurrences(of: "context", with: "e") + "onUid")] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(String(kStr_dataValue))] = userInfoModel.uid
        }
    ...
    func withToolClick() {
        ...
        queue.async(group: group) {
            ...
            SomeoneThen.misquote(uid: self.uid) { succeed, result, _ in
                //: self.setNoremarlView()
                self.asPickView()
                //: if self.uid == LocationThen.share.loginUid {
                if self.uid == LocationThen.share.loginUid {
                    //: self.bottomView.showEditBtn()
                    self.bottomView.checked()
                }
                //: if succeed == true {
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<LoadMeasurable>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        self.navView.eventFrom(userModel)
                        //: self.headerView.refreshView(userModel)
                        self.headerView.animalView(userModel)
                        //: self.bottomView.refreshView(userModel)
                        self.bottomView.perspectiveApplication(userModel)
                        //: self.postVC.refreshPostViewController(userModel)
                        self.postVC.action(userModel)
                        ...
                        self.roomHeadImgView.failureFinish(urlStr: userModel.headPic, options: optionsInfo)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            SomeoneThen.playerPush(uid: self.uid) { succeed, result, _ in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = DeleteTransformable.deserialize(from: dic as? Dictionary) {
    ...
    private func close() {
        ...
        var profileSectionArr = [PointCaseIterable.profileType]
        ...
        self.profileVC.populationProfile(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension ClientShowViewDelegate {
    ...
    @objc private func betweenModel() {
        //: req_followRequest()
        enableAction()
    }
    ...
    private func content() {
        //: self.followBtn.isHidden = (userInfoModel.uid == LocationThen.share.loginUid)
        self.followBtn.isHidden = (userInfoModel.uid == LocationThen.share.loginUid)
        //: let followImgStr = self.userInfoModel.isAttention ? "bth_me_follow_pre":"bth_me_follow_nor"
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: kStr_gameAddName.map{makeFalse(the: $0)}, encoding: .utf8)! : (String(kStr_modelValue) + String(kStr_freshValue) + kStr_skipText.replacingOccurrences(of: "value", with: "l") + "ow_nor")
        //: self.followBtn.image = UIImage.bundleBy(name: followImgStr)
        self.followBtn.image = UIImage.bundleBy(name: followImgStr)
    }

    /// 展示VIP皮肤视图
    //: private func setupVIPSkinView() {
    private func capacityView() {
        //: guard userInfoModel.loungePlus else { return }
        guard userInfoModel.loungePlus else { return }
        //: guard let vipModel = LocationThen.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first else { return }
        guard let vipModel = LocationThen.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first else { return }
        ...
        vipCenterView.timeFinish(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.timeFinish(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension ClientShowViewDelegate: JXSegmentedViewDelegate {
...
extension ClientShowViewDelegate: JXPagingViewDelegate {
...
extension ClientShowViewDelegate {
    ...
    @objc private func conversationMethod() {
        //: requestAllData()
        withToolClick()
    }
}
...
extension ClientShowViewDelegate {
    ...
    private func asPickView() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(kLet_clickLimitTitle)
        }
    }

    /// 添加约束
    //: private func setupSubViewsConstraint() {
    private func tillCollection() {
        //: pagerView.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight)
        pagerView.frame = CGRect(x: 0, y: 0, width: kLet_halfData, height: kLet_noSeatName)
        ...
        voiceRoomBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-mediaBlock(h: 178) - kLet_formatNetValue)
    ...
    private func data() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(conversationMethod),
    ...
    private func quantityimate() {
        ...
        voiceRoomBtn.rx.tap
            ...
            .subscribe(onNext: { [weak self] in
                //: guard let self = self else {return}
                guard let self = self else { return }
                //: EffectPushManager.share.func_pushToVoiceRoomVC(roomId: self.userInfoModel.partyRoomId.stringValue)
                EffectPushManager.share.formerId(roomId: self.userInfoModel.partyRoomId.stringValue)
                //: })
            })
