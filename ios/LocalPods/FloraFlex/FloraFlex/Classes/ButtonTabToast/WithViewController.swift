...
fileprivate let kStr_renderData:[UInt8] = [0xc8,0xcf,0xc8,0xd5,0x89,0xc2,0xce,0xc5,0xc4,0xd3,0x9b,0x88,0x81,0xc9,0xc0,0xd2,0x81,0xcf,0xce,0xd5,0x81,0xc3,0xc4,0xc4,0xcf,0x81,0xc8,0xcc,0xd1,0xcd,0xc4,0xcc,0xc4,0xcf,0xd5,0xc4,0xc5]
...
class WithViewController: LayerRecognizerDelegate {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: CanPresentTransition!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = CanPresentTransition(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kStr_renderData.map{$0^161}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class CanPresentTransition: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: kLet_halfData, height: kLet_noSeatName * 0.6)
    ...
    func conversionError(corners: UIRectCorner, radius: CGFloat) {
...
extension CanPresentTransition: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = SquareBlockRecognizerDelegate(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension CanPresentTransition: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            upMic(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            sumeriorFemale(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func upMic(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func sumeriorFemale(transitionContext: UIViewControllerContextTransitioning) {
...
class SquareBlockRecognizerDelegate: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: CanPresentTransition!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: CanPresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func colored() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(tillObserver))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: kLet_halfData, height: kLet_noSeatName)
