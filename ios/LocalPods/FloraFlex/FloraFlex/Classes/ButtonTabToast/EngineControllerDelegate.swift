...
class EngineControllerDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: PubSendable?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.nextItem()
            }
        }
    ...
    func navibar() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.locationAcross(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.bottomValue()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.dataFormat(), NSAttributedString.Key.font: UIFont.groupSize(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.locationAcross(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.dataFormat(), NSAttributedString.Key.font: UIFont.groupSize(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.dataFormat()
...
extension EngineControllerDelegate: UINavigationControllerDelegate {
...
extension EngineControllerDelegate: UIGestureRecognizerDelegate {
