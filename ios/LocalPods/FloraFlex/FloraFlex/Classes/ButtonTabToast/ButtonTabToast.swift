...
fileprivate let kStr_labDataTitle:[UInt8] = [0x31,0x36,0x31,0x2c,0x70,0x3b,0x37,0x3c,0x3d,0x2a,0x62,0x71,0x78,0x30,0x39,0x2b,0x78,0x36,0x37,0x2c,0x78,0x3a,0x3d,0x3d,0x36,0x78,0x31,0x35,0x28,0x34,0x3d,0x35,0x3d,0x36,0x2c,0x3d,0x3c]

private func tableMagnitude(original num: UInt8) -> UInt8 {
    return num ^ 88
}

/*: "btn_live_toast_bg" :*/
fileprivate let kStr_textAvailableTitle:[Character] = ["b","t","n","_","l","i","v","e","_","t","o","a","s","t","_","b","g"]

/*: "btn_live_toast_arrow" :*/
fileprivate let kStr_centerFatalTitle:String = "array selected imagebtn_l"
fileprivate let kStr_byName:[Character] = ["t","o","a","s","t","_","a","r","r","o","w"]
...
var kLet_homeData = false

//: class TalkingLiveTabToast: UIView {
class ButtonTabToast: UIView {
    // 气泡间距
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStr_labDataTitle.map{tableMagnitude(original: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bubbleImgView: UIImageView = {
        ...
        let imgV = UIImageView()
        //: var img = UIImage.bundleBy(name: "btn_live_toast_bg").resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        var img = UIImage.bundleBy(name: (String(kStr_textAvailableTitle))).resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        //: imgV.image = img
        imgV.image = img
        //: return imgV
        return imgV
    ...
    private lazy var arrowImgView: UIImageView = {
        //: let imgV = UIImageView()
        let imgV = UIImageView()
        //: imgV.image = UIImage.bundleBy(name: "btn_live_toast_arrow")
        imgV.image = UIImage.bundleBy(name: (String(kStr_centerFatalTitle.suffix(5)) + "ive_" + String(kStr_byName)))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension ButtonTabToast {
    /// 展示视图，5s后自动移除
    //: func show(superView: UIView) {
    func dataFormattingView(superView: UIView) {
        //: guard LIVETABTOASTISSHOW == false else { return }
        guard kLet_homeData == false else { return }
        ...
        //: guard LocationThen.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        guard LocationThen.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        //: LIVETABTOASTISSHOW = true
        kLet_homeData = true

        //: textLab.text = LocationThen.share.appUserConfigMode.liveTabTips
        textLab.text = LocationThen.share.appUserConfigMode.liveTabTips
        //: let maxWidth = ScreenWidth - 48
        let maxWidth = kLet_halfData - 48
